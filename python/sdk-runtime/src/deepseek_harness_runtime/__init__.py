"""Locate the bundled Tianshu Harness SDK runtime shipped with this package.
...
- **exe (production)**: single-file Node executables named
  ``dsh-sdk-server-agent-pkg-<platform>-<arch>`` (platform in {linux, macos}, arch in
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@huiliyi37/dsh-jsonrpc-demo/lib/bin.js`` on a
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"dsh-sdk-server-agent-pkg-{tag}"
    if not path.is_file():
        raise FileNotFoundError(
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported Tianshu Harness runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if plat is None or arch is None:
        raise FileNotFoundError(
            "no bundled dsh-sdk-server-agent executable exists for this platform "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    bin_js = (
        node_root / "node_modules" / "@huiliyi37" / "dsh-jsonrpc-demo" / "lib" / "bin.js"
    )
