...
| `ctx.modelRoles` | [`dsh-model-roles`](../packages/core/model-roles/README.md) | Settings-backed model pins for the vision, secondary, and subagent roles |
...
The session log is authoritative. `deriveMessages()` projects model history; raw `assistant/chunk` events preserve replay and UI fidelity. Fork, resume, transcript rendering, telemetry, and persistence derive from this stream. Truncation rewinds the log with derived-state reset.
...
`dsh-agent-spine-demo` bundles a spine and optional goals. App packages own CLI, ACP automation, and JSON-RPC front doors ([README](../packages/examples/agent-spine-demo/README.md), [acp/](../packages/acp/README.md), [interaction/](../packages/interaction/README.md)). `dsh-sdk-server-agent` boots external `cordis.yml`; the Python SDK defaults when config is absent ([Python SDK](../python/README.md)). Thin deployments use swappable backends and optional tools ([examples/](../examples/AGENTS.md), [runnable wirings](cookbook/extension-cookbook.md#runnable-wirings), [graph atlas](graph-atlas.md)).
...
| Enforce verification or routing discipline | register a guard plugin on tool/agent events (`guard/` family) |
...
| Rewind a session's log | call `Session.truncate`; persistence backends implement `deleteFrom` |
