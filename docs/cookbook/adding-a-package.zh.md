...
为新建 `@huiliyi37/dsh-<name>` 包提供的逐文件清单。本清单以 bash 和 adapter 这两个包为模板进行验证；如果清单与模板有出入，请在此修正。
...
| 文件 | 变更 |
|---|---|
| `tsconfig.base.json` | 已有分组无需编辑；新分组需为 `@huiliyi37/dsh-*` 通配符添加 `./packages/<group>/*/src` 候选路径 |
