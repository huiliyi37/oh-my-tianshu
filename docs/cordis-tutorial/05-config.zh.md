...
`!!js` 仅在 `config` 内和条目的 `disabled` 字段上有效。`disabled: !!js ...` 在每次挂载决策时基于 loader 上下文求值（本仓库的扩展），因此一行可以按平台或环境门控自身；其余元数据（`name`、`id`、`inject` 等）是静态的，表达式在那里只是普通的真值数据。详见 [loader 配置](../cordis-primer.md#loader-configuration)。
