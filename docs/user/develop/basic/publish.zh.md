...
前几篇教程通过 `--patch` overlay 加载本地插件。本教程把它打包成可安装的**组合包**，用 `oh-my-tianshu plugin add` 安装进一个 **profile**，并解释决定组合后配置的层顺序。请先完成[插件配置](./config.md)。
...
组合包是你编写并分发的东西；profile 是用户用 `oh-my-tianshu --profile <name>` 启动的东西。没有东西同时是两者。
...
没有 `dsh.bundle` 声明的包仍然可以安装，但只作为普通依赖：`oh-my-tianshu plugin` 会打印警告，且不激活任何层。这正是「供插件包 import 的库」应有的形状，区别于「供用户启用的插件」。
...
profile manifest 从不需要手写：`oh-my-tianshu plugin` 负责创建和维护它。下一节展示其结果。

## 安装进 profile

`oh-my-tianshu plugin --profile <name> <args...>` 在 profile 目录内转发给 pnpm，因此所有 pnpm 子命令都可用。从 checkout 安装你的包：
...
`oh-my-tianshu plugin --profile demo remove dsh-hello-plugin` 会同时移除依赖和对应的层。
...
5. 启动器 flag patch（例如 `oh-my-tianshu web --port`）。
...
- **发布到 npm**，在 `pnpm publish` 时构建好 `lib/`；`oh-my-tianshu plugin add your-package` 安装的就是预构建代码。
- **交付 tarball**：用 `pnpm pack` 打包；用户执行 `oh-my-tianshu plugin add ./hello-plugin-0.1.0.tgz`。
