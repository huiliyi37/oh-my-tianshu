...
Call `next()` for the default or return a decision to short-circuit. Pre-policy may deny or ask; `allowed-once` or `allowed-always` proceeds, while a non-grant, missing approval channel or service, or agent-less request becomes a denial. Guards may still impose a final denial. Arguments cannot be rewritten because history, audit, UI, and execution must agree.
...
Source: [`packages/guard/zen/src/index.ts:407`](../../packages/guard/zen/src/index.ts)
