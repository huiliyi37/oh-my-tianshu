...
[`@huiliyi37/dsh-message-feedback`](../../packages/feedback/message-feedback) owns editable feedback for individual assistant messages. It is deliberately separate from the immutable Session-level `feedback/record` event: message feedback is a local storage-domain sidecar, not Session-log content or a projection, and it performs no telemetry handoff.
...
The official browser consumer (`ui-message-feedback`) is not ported: the local web surface is a self-built counterpart and the client panel waits for a consumer confirmation. The Host Remote contract above is available now, and a future thin client adapter mounts it through `ctx.remote.messageFeedback` without touching persistence or concurrency semantics.
