...
extensions 子系统允许 agent（智能体）定义带版本的 Cordis 包、运行其 host 与浏览器两半，并在编写代码前查询获准公开的运行时元数据。包生命周期与沙箱行为由 [`packages/self-modification`](../../packages/self-modification/README.md) 包组说明；[按会话 agent 预设说明](../../.agents/notes/implemented/feature/2026-08-03-per-session-agent-presets.md) 负责预设驱动的组合边界。
