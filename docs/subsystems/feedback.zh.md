...
[`@huiliyi37/dsh-message-feedback`](../../packages/feedback/message-feedback)拥有针对单条 assistant 消息的可编辑反馈。它刻意与不可变的 Session 级 `feedback/record` 事件分离：message feedback 是本地 storage-domain 伴随记录（sidecar），不是 Session 日志内容或投影，也不执行遥测交接。
...
官方浏览器消费方（`ui-message-feedback`）未移植：本地 web 面是自研对应物，client 面板待消费方确认。上述 Host Remote 契约现成可用，未来薄 client 适配器经 `ctx.remote.messageFeedback` 挂载即可，不触碰持久化或并发语义。
