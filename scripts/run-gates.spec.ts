...
describe('gate graph validation', () => {
  ...
  it.each(['ci-static', 'check-all'] as const)(
    'keeps the welcome fox projection check in %s',
    (mode) => {
      const ids = withPnpmEntrypoint(() => gatesForMode(mode).map(subject => subject.id))

      expect(ids).toContain('welcome-fox')
    },
  )
...
describe('isEntryPoint', () => {
  it('matches argv[1] against this module URL (entry point)', () => {
    // tsx/type-stripping never populate import.meta.main — the previous guard
    // silently skipped main() and no gate ever ran. The argv comparison is the
    // replacement contract.
    const moduleUrl = 'file:///repo/scripts/run-gates.ts'
    expect(isEntryPoint('/repo/scripts/run-gates.ts', moduleUrl)).toBe(true)
    // A relative argv[1] resolves against the cwd — the real module path must
    // match its own URL.
    expect(isEntryPoint(fileURLToPath(import.meta.url), import.meta.url)).toBe(true)
  })

  it('returns false when imported as a module or for another file', () => {
    const moduleUrl = 'file:///repo/scripts/run-gates.ts'
    expect(isEntryPoint(undefined, moduleUrl)).toBe(false)
    expect(isEntryPoint('/repo/scripts/run-gates.spec.ts', moduleUrl)).toBe(false)
    expect(isEntryPoint('/other/scripts/run-gates.ts', moduleUrl)).toBe(false)
  })
})
