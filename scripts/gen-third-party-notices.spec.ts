...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/support/loader-smoke/package.json': { name: '@huiliyi37/dsh-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/client/test-runtime/package.json': { name: '@huiliyi37/dsh-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@huiliyi37/dsh-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@huiliyi37/dsh-cli', dependencies: { 'cli-lib': '^1', '@huiliyi37/dsh-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@huiliyi37/dsh-tui', dependencies: { shared: '^1', '@huiliyi37/dsh-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@huiliyi37/dsh-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@huiliyi37/dsh-cli')).toBe(false)
  })
})
