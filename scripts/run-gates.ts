...
/**
 * Entry-point detection. `import.meta.main` is `undefined` under tsx and
 * Node's type-stripping runner (verified on Node 24.1.0) — the previous
 * `if (import.meta.main)` guard silently skipped `main()` entirely, so every
 * `check:ci` / `check:all` / CI gate invocation exited 0 with zero output and
 * no gate ever ran. Compare argv[1] against this module's URL instead.
 * @param argv1 - `process.argv[1]` (undefined when this module is imported).
...
function docSyncLeafGates(options: {
  ...
  return [
    ...
    pnpmScript('capability-coverage', 'verify-capability-coverage', { label: 'capability coverage' }),
...
function printResult(result: GateResult): void {
  ...
  const writeHeading = result.status !== 'passed' ? console.error : console.log
  writeHeading(`\n== ${heading} ==`)
