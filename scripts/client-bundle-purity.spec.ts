...
function clientConfigs(id = '@huiliyi37/dsh-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@huiliyi37/dsh-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@huiliyi37/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@huiliyi37/dsh-client-web-react')).toBeNull()
    expect(resolveId('@huiliyi37/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@huiliyi37/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@huiliyi37/dsh-host-apiproxy/api')).toBeNull()
    expect(resolveId('@huiliyi37/dsh-session/surface')).toBeNull()
    expect(resolveId('@huiliyi37/dsh-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@huiliyi37/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@huiliyi37/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@huiliyi37/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@huiliyi37/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @huiliyi37 leak', () => {
    expect(() => { return resolveId('@huiliyi37/dsh-agent') }).toThrow(/purity/)
    expect(() => resolveId('@huiliyi37/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@huiliyi37/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@huiliyi37/dsh-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@huiliyi37/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@huiliyi37/dsh-client-runtime/client')).toBeNull()
    const dshClientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@huiliyi37/') && entry.endsWith('/client'))
    expect(dshClientChannels).toEqual(['@huiliyi37/dsh-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@huiliyi37/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@huiliyi37/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@huiliyi37/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@huiliyi37/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@huiliyi37/dsh-client-connection/client.js.map')
