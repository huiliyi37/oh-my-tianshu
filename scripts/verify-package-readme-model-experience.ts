...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/host/plugin-inventory': { kind: 'none', reason: 'The Host-only inventory projection registers no prompt, tool, message, or provider request.' },
  'packages/identity/anonymous-user-id': { kind: 'none', reason: 'The identity is model-hidden transport metadata and never enters a request body, prompt, or model-visible content.' },
  ...
  'packages/llm/cache-diagnostic': { kind: 'indirect', reason: 'The observability service reads the durable log and reports to UIs/logs; it registers no model surface of its own.' },
  ...
  'packages/subagent/agent-definitions': { kind: 'indirect', reason: 'The role registry delegates catalog and persona rendering to dsh-tool-subagent and the subagent seam.' },
