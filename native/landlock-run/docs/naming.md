...
The public package family belongs to the `@huiliyi37` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@huiliyi37/node-addon-landlock-run
@huiliyi37/node-addon-landlock-run-<platform>
```
