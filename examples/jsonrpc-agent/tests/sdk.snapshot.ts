...
import { resolveExampleLaunch } from '@huiliyi37/dsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@huiliyi37/dsh-sdk-client'
...
function assembledPolicyContext(log: PersistedLog): string {
  const contexts = log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@huiliyi37/dsh-system-prompt') return []
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  try {
    const notifications: Array<HarnessNotification> = []
