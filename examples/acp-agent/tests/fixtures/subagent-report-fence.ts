...
import type {} from '@huiliyi37/dsh-agent-loop'

/** Fixture plugin name. */
export const name = 'subagent-report-fence'

/**
 * Keep replay scheduling from folding the next-step report into the parent's
 * first turn: the child waits for the spawn turn to close, so the waking
 * report starts one deterministic parent turn of its own.
