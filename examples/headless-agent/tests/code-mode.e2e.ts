...
import LlmService, { createUserMessage, CallId, HarnessError  } from '@huiliyi37/dsh-llm'
import SessionStore, { SessionId } from '@huiliyi37/dsh-session'
import type { SessionEvent } from '@huiliyi37/dsh-session'
import SystemPrompt from '@huiliyi37/dsh-system-prompt'
import ToolRegistry, { RUN_CODE_NAME, defineTool } from '@huiliyi37/dsh-tools'
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('Code Mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [run_code], bridges sub-calls, and returns curated output', async () => {
    ...
    for (const header of headers) expect(header.data.header.tools?.map(tool => tool.name)).toEqual([RUN_CODE_NAME])
