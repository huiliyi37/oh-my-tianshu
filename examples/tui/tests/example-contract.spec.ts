...
describe('examples/tui example contract', () => {
  it('examples/package.json declares @deepseek-ai/dsh-tui as workspace:*', () => {
    const pkg = JSON.parse(readFileSync(resolve(examplesRoot, 'package.json'), 'utf8')) as {
      dependencies?: Record<string, string>
    }
    expect(pkg.dependencies?.['@deepseek-ai/dsh-tui']).toBe('workspace:*')
  })

  it('examples/tui/cordis.yml references the @deepseek-ai/dsh-tui bundle', () => {
    const cordis = readFileSync(resolve(examplesRoot, 'tui', 'cordis.yml'), 'utf8')
    expect(cordis).toContain('@deepseek-ai/dsh-tui')
  })
