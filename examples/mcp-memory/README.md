...
These three **default-off reference configurations** connect one memory system to DSH through [`@huiliyi37/dsh-mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.
...
```sh
oh-my-tianshu web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
oh-my-tianshu web --patch "${DSH_HOME:-$HOME/.dsh}/memory.cordis.yml"
```
...
oh-my-tianshu web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
oh-my-tianshu web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
