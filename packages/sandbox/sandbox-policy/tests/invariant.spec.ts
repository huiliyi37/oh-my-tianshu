...
import SessionStore, { type Session, type SessionEvent } from '@huiliyi37/dsh-session'
import InvariantService, { InvariantError } from '@huiliyi37/dsh-invariants'
import * as SandboxPolicyInvariant from '@huiliyi37/dsh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@huiliyi37/dsh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => { return undefined })).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@huiliyi37/dsh-sandbox-policy',
    })
  })
})
