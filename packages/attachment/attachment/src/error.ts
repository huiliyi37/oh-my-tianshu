/** Attachment failure class. @module @huiliyi37/dsh-attachment/error */
 ...
 * the base lives in `@huiliyi37/dsh-llm`, and this seam stays dependency-free
 * so provider packages can reference `ImageAttachmentRef` without a dependency
 * cycle. Consumers route on `code`, never on the prototype chain, so the
 * shapes stay interchangeable at the wire boundary.
 */
