...
| [`schedule/`](schedule/README.md) | 会话内持久提醒 | 产品：稳定接口 |
...
| [`plan/`](plan/README.md) | Plan 协作状态：直接进入命令，经评审退出 | 产品：稳定接口 |
| [`preset/`](preset/README.md) | 基于 preset cordis.yml 文件的按会话 agent 组成 | 产品：稳定接口 |
| [`guard/`](guard/README.md) | 循环卫生守卫：重复调用提醒、工具截止时间、zen 初始 face、任务卡改写、意图对齐桥交接 | 产品：稳定接口 |
