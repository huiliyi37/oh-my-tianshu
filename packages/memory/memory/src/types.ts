...
/** 记忆事件种类（结构化 LTM 的事件分类；缺省 'observation'，带结构化 fact 时缺省 'fact'）。 */
...
/**
 * save 的输入：基础条目字段 + 可选结构化字段。结构化字段由结构化 provider
 * （dsh-memory-sqlite）消费；Markdown provider 忽略它们（纯文本存储）。
 */
...
export interface MemorySearchOptions {
  ...
  /** 跳过前 N 条。 */
  offset?: number
  /** 按 id 或 id 前缀排除条目（供调用方排除已进入 STM 等上下文、无需重复的条目）。 */
  excludeIds?: string[]
...
export interface MemoryService {
   ...
   * 无得分；SQLite provider 为 BM25 + 实体 + 时序混合检索，带归一化 score）。
   ...
   * `typeof memory.topicVersions === 'function'` 探测（adaptive-memory 的
   * STM 门控消费此信号）。
