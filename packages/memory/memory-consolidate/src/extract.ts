...
function statedFact(body: string): MemoryFactShape | undefined {
  for (const rule of STATED_RULES) {
    ...
    if (match !== null) {
      ...
      const value = match[2]
      /* v8 ignore next -- 捕获组在本正则命中时恒有 */
      if (subject === undefined || value === undefined) continue
      return { subject: subject.trim(), predicate: 'stated', value: value.trim() }
    }
  }
  return undefined
}
 ...
 * @param text - 待截断文本。
 * @param maxTextChars - 字符上限。
 * @returns 截断后的文本（超限时尾部为省略号）。
 */
