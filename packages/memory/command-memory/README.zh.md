...
基于可选的 [`memory` 服务](../memory/README.md)提供面向用户的 `/remember` 与 `/memory` 命令。该插件通过 [`ctx.commands`](../../interaction/commands/README.md) 注册这两个命令，因此组合中的每个命令适配器——包括 Web 斜杠菜单——都能发现并执行它们，无需模型轮次。TUI 拥有自己的注册表条目；本包是宿主侧的对应物。
...
随附的 `dsh` 基础配置挂载本插件但不挂载 memory 服务，因此这两个命令会出现在每个命令适配器中，并在组合加入 `dsh-memory` 之前一直回答不可用文本。
