...
describe('intentKeyOf', () => {
  ...
  it('空动词不匹配；image 块不进锚点文本', () => {
    const session = Session.create(SessionId('intent-empty-verb'))
    session.append('turn/start', { turn: 1 })
    session.append('user/message', createUserMessage({
      content: [
        { type: 'image', dataUrl: 'data:image/png;base64,xx' },
        { type: 'text', text: 'please look at login' },
      ],
      source: { kind: 'user' },
    }), { surfaceOp: 'append' })
    expect(findGoalAnchor(session.events, ['', 'fix'])?.text).toBe('please look at login')
    session.append('turn/start', { turn: 2 })
    session.append('user/message', createUserMessage({
      content: [
        { type: 'image', dataUrl: 'data:image/png;base64,yy' },
        { type: 'text', text: 'fix the payment module' },
      ],
      source: { kind: 'user' },
    }), { surfaceOp: 'append' })
    expect(findGoalAnchor(session.events, ['', 'fix'])?.anchorIndex).toBe(2)
  })
})
...
describe('extractEntities', () => {
  ...
  it('过长实体丢弃；非文本 tool/result 内层不提取错误码文本', () => {
    const session = Session.create(SessionId('intent-long'))
    appendUserTurn(session, 1, 'fix the login bug')
    const anchor = findGoalAnchor(session.events, GOAL_VERBS)
    if (anchor === undefined) throw new Error('missing anchor')
    const longPath = `${'dir/'.repeat(80)}file.ts`
    session.append('tool/call', {
      turn: 1, step: 2, callId: CallId('call-long'), name: 'read_file', arguments: `{"path":"${longPath}"}`,
    })
    session.append('tool/result', {
      turn: 1,
      step: 2,
      ...
      error: { name: 'IoError', code: 'EACCES' },
    }, { surfaceOp: 'append' })
    expect(extractEntities(session.events, anchor.seq, 24)).toEqual(['EACCES'])
  })
})
