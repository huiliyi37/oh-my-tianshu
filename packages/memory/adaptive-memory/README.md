...
- Mounting this plugin without `@huiliyi37/dsh-memory` is a configuration error and fails loud at the first evaluation; `confidenceHigh < confidenceMedium` fails loud at load.

All thresholds are Config fields with schema defaults: `stmTokenBudget` (600), `maxEntries` (12), `maxIntentTokens` (6), `maxEntities` (24), `reviewIntervalTurns` (8), `goalVerbs`, `alwaysIncludeTags` (`['safety', 'constraint', 'preference']`), `summaryMaxChars` (120), `maxKeywords` (5), `confidenceHigh` (0.82), `confidenceMedium` (0.55), `retrievalLimit` (24), `maxRemindersPerTurn` (1), `maxRemindersPerIntent` (3).
