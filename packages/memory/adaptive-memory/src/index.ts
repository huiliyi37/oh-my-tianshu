...
async function reviewStm(
  ...
  if (reason === undefined) {
    /* v8 ignore next -- refreshReason 无快照时返回 'initial'，不会走到 cache-hit */
    if (prev === undefined) throw new Error('adaptive-memory: cache-hit without STM snapshot')
    prev.evaluatedTurn = turn
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    /* v8 ignore next -- context() 注册的槽在 PromptAssembly 恒在 */
    if (entry === undefined) throw new Error('adaptive-memory: memory:stm context 槽缺失')
    entry.text = text
    ...
    /* v8 ignore next -- 同上，reminder 槽恒在 */
    if (reminderEntry === undefined) throw new Error('adaptive-memory: memory:reminder context 槽缺失')
    reminderEntry.text = reminderText
    return result
  })
}
