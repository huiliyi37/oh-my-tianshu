...
describe('SqliteMemoryStore 向量层', () => {
  ...
  it('空库有 embedder 的查询不取向量', async () => {
    const embedder = makeFakeEmbedder('fake-v1', BILLING.concepts)
    const { store } = await makeStore({ embedder })
    expect(await store.search(BILLING.query)).toEqual([])
    expect(embedder.calls).toEqual([])
  })

  it('embedder 返回数量不符 fail loud', async () => {
    ...
    const { store } = await makeStore({ embedder })
    await expect(store.save({ text: 'x', scope: 'global', tags: ['t'], source: 'agent' }))
      .rejects.toThrow(/returned 0 vector/)
  })

  it('惰性重嵌若某条向量缺失 fail loud', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-memory-sqlite-embed-'))
    dirs.push(dir)
    const dbPath = join(dir, 'ltm.sqlite')
    const writer = new SqliteMemoryStore({ dbPath, journalMode: 'wal', importMaxFileBytes: 1_048_576 })
    ...
    const embedder: EmbeddingProvider = {
      id: 'hole',
      isAvailable: () => true,
      embed: texts => Promise.resolve(texts.map((_, index) => (
        index === 0 ? Array.from({ length: FAKE_DIM }, () => 1) : undefined
      )) as number[][]),
    }
    const store = new SqliteMemoryStore({ dbPath, journalMode: 'wal', importMaxFileBytes: 1_048_576, embedder })
    stores.push(store)
    await expect(store.search(BILLING.query)).rejects.toThrow(/fewer vectors than requested/)
  })
})
