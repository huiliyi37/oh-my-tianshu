...
import { createLlmKeywordExpander, parseExpansionOutput } from '../src/expander.ts'
...
describe('memory-sqlite 关键词扩展（阶段二d）', () => {
  ...
  it('createLlmKeywordExpander：截断超长输入并解析 invoke 输出', async () => {
    const users: string[] = []
    const expand = createLlmKeywordExpander({
      maxInputChars: 8,
      route: { provider: 'p', model: 'm' },
      invoke: (request) => {
        users.push(request.user)
        expect(request.route).toEqual({ provider: 'p', model: 'm' })
        expect(request.system.length).toBeGreaterThan(0)
        return Promise.resolve('["alpha", "beta"]')
      },
    })
    expect(await expand({ text: 'abcdefghij', keywords: ['note'], topic: 'note' })).toEqual(['alpha', 'beta'])
    expect(users[0]).toContain('truncated at 8 chars')
    expect(await expand({ text: 'short', keywords: ['note'], topic: 'note' })).toEqual(['alpha', 'beta'])
    expect(users[1]).toContain('Entry:\nshort')
    expect(users[1]).not.toContain('truncated')
  })
})
