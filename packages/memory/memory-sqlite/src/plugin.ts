 ...
 * consumers（tool-memory、TUI 命令）经
 ...
 * 聚合文本块）。llm 服务在调用时探测（'llm' 而未装配 llm 服务 = 本次扩展失败，
 * store 记录后按未扩展落库）。扩展是机械生成：缺省关思考，避免推理模型把输出
 * 预算烧在思考 token 上。
