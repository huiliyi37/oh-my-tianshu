 ...
 * registers is misconfiguration and fails loud. Narrowing prunes the prompt
 * with the face: every assembly drops each `tool:<name>` section whose tool
 * is off that assembly's face (see `./tool-sections.ts`).
 *
...
export class ZenPhaseService extends Service {
  ...
  constructor(ctx: Context, config: ZenConfig = { section: '' }) {
    ...
    // the first request/header already carries the anchored face. The registry
    // may still be filling in here (a front door that attaches on its own
    // services reaches this seam before plugins awaiting an injected service
    // register theirs), so arming takes the registered subset and owes the
    // rest to the first per-agent seam — see completeArm.
