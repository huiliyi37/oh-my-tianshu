...
Service dependencies: `sessions`/`agents`/`agentDefaultModel` are required; `goals`/`subagents` are optional — when unassembled, the `/goal` command and the delegation-tree panel degrade (fails loud reporting unavailability, never swallowing silently). The TUI also registers the `userInteraction` provider (in-terminal question answering) and subscribes to `approval/request` (pending approval cards).
...
- **Image re-interrogation deferred** — the opencode-tui ask_image tool, image registry, and vision description cache are not ported: an already-sent image cannot be re-queried, and repeated same-angle descriptions re-call the vision model. The vision bridge itself (`dsh-vision-bridge`) covers the one-shot submit-time description path.
