...
**API key setup** (`/key`, alias `/login`): a masked-input dialog probes the key against the DeepSeek API (`401/403` rejects, network errors allow saving anyway) and persists it through the `credentials` service (`$DSH_HOME/.credentials.yaml`, mode 0600) — resolution is per request, so a saved key takes effect without a restart. On interactive startup with no key configured, the dialog opens once automatically (Esc skips); a process-environment `DEEPSEEK_API_KEY` shadows the file layer, and the dialog explains that instead of writing.
