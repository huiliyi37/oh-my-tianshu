...
**Session recovery visibility** (session-resume): cold start renders a numbered restorable-session list (title · age · cwd) in the welcome card — digits 1–9 resume the numbered session while the welcome phase is active (any typed character exits the phase), `ctrl+s` resumes the most recent other session, and `/resume [id]` resumes the most recent session or a named one. Resuming mounts a banner (title · last activity · cwd) and a `上次进行到此处` separator after the replayed history, plus an interrupted-run notice when the log carries crash-repair closers. `dsh tui --session <id>` and `dsh run --session <id> "task"` resume a named session from the command line; unknown ids fail loud with recovery guidance. Corrupt persisted artifacts stay listed as `不可恢复` instead of disappearing, and the session tab row always shows the current session (short id + ●) even with one session.
