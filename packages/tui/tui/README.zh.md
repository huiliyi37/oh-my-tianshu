...
**错误终态恢复**（移植自 opencode-tui 的 handleError 三件套）：回合以 `error` 终态收尾时，转录追加失败摘要与一行分类「下一步」（`error-recovery.ts` 按结构化 `LlmFailure` 事实分流——认证 → `/key`、限流/额度 → 等待或切轻量档、5xx → 换供应商、上下文超限 → `/compact`、网络 → `/doctor` 等），并把最近一条真实用户消息回填输入轨、附告知行（`↩ 上一条可能未被完整处理`）——输入框已有草稿时不抢写。回填底料在成功终态与手动打断时清空（打断路径自有草稿保留语义）。`/doctor` 尾部附排障指针行。
...
**模型选择器同行档位调节**（`/model`，移植自 opencode-tui 面板）：模型列表对选中行惰性解析推理档位（`ctx.llm.resolveModelInfo`，按路由缓存）；当前档位以行内 `档位` 位显示，`<`/`>` 环形步进，Enter 时模型与步进后的档位一次 `saveSelection` 持久化（未步进的行保持既有语义——只切模型不动档位）。无档位目录的行按键静默，footer 仅在步进可用时提示键位。
...
**设置面板**（`/config`）：交互式双栏 framed overlay（左类目右字段，对标竞品 `/config` 的交互词汇）。类目：模型（默认模型、推理档位、三角色 pin——Enter 打开与命令同一套选择器）、子代理模型（子路由授权，装配 `subagent-model-selection-settings` 入口时出现：启用开关与精确授权路由；尚无授权路由时类目置顶「一键推荐」行（flash 命名模型优先，其次 deepseek 系，再次目录首个模型），每条路由行在 llm 活目录无法解析时追加 ⚠ 提示（advisory——授权本身不撤销）；选择关闭且无路由时打开 `/subagents` 每会话提示一次去向；逐行移除、新路由经 provider/model 两级 picker 从 llm 活目录添加；写入走 settings 文档，变更只影响尚未记录策略的会话）、权限（预设切换，走 `/permission` 同一写路径）、凭据（各供应商 key 状态；Enter 直接进入该供应商的 `/key` 对话框）、概览（只读的已解析 settings 命名空间与脱敏标记）。编辑即时生效——所有写面（角色 pin、默认模型选择、权限 apply、凭据 set）都热生效，因此刻意不做草稿/脏块/保存机制；编辑器关闭后面板回到原字段回开。
...
交互式双栏 framed overlay（左类目右字段，对标竞品 `/config` 的交互词汇）。类目：模型（默认模型、推理档位、三角色 pin——Enter 打开与命令同一套选择器）、子代理模型（子路由授权，装配 `subagent-model-selection-settings` 入口时出现：启用开关与精确授权路由；尚无授权路由时类目置顶「一键推荐」行（flash 命名模型优先，其次 deepseek 系，再次目录首个模型），每条路由行在 llm 活目录无法解析时追加 ⚠ 提示（advisory——授权本身不撤销）；选择关闭且无路由时打开 `/subagents` 每会话提示一次去向；逐行移除、新路由经 provider/model 两级 picker 从 llm 活目录添加；写入走 settings 文档，变更只影响尚未记录策略的会话）、权限（预设切换，走 `/permission` 同一写路径）、凭据（各供应商 key 状态；Enter 直接进入该供应商的 `/key` 对话框）、概览（只读的已解析 settings 命名空间与脱敏标记）。编辑即时生效——所有写面（角色 pin、默认模型选择、权限 apply、凭据 set）都热生效，因此刻意不做草稿/脏块/保存机制；编辑器关闭后面板回到原字段回开。
...
- **用户级 TTY 验收受阻** — 代理沙箱无法驱动真实终端做人工验收；行为证据以单测与真实组合测试为准。
- **投影模型部分接线** — turn-summary 与 summary-state 已由 App 主体驱动（turn 结束摘要行；`/compact` 直读 summary-state），activity-status/activity-store 已带 spec 落地为纯 fold 但尚无驱动方（仅 fluency 链消费 `ActivityPhase` 类型）；设计中的 cache-telemetry/cache-panel-source/history-replay/adapter-projections 未落地。现状记录在 [docs/projection-layer.md](docs/projection-layer.md)。
（`/model`，移植自 opencode-tui 面板）：模型列表对选中行惰性解析推理档位（`ctx.llm.resolveModelInfo`，按路由缓存）；当前档位以行内 `档位` 位显示，`<`/`>` 环形步进，Enter 时模型与步进后的档位一次 `saveSelection` 持久化（未步进的行保持既有语义——只切模型不动档位）。无档位目录的行按键静默，footer 仅在步进可用时提示键位。
