...
describe('TuiApp Phase 8 审批 answerer', () => {
  it('当前会话请求 → 挂起提示 + y 放行（allowed-once）', async () => {
    ...
    expect(written).toContain('[a] 本会话总是允许')
  ...
  it('审批挂起按 a → 本会话总是允许（always-approve + 当前请求 allowed-always）', async () => {
    const ctx = makeCtx()
    ...
    await expect(next).resolves.toBe('allowed-always')
    await app.dispose()
  })

  it('审批挂起按 p → 经 approvalRules.persistAllow 落规则并以 allowed-always 结算', async () => {
    const ctx = makeCtx()
    const agent = makeAgent('approval-p')
    const handle = makeHandle(agent)
    ctx.agents.create.mockResolvedValue(handle)
    ctx.sessions.get.mockReturnValue(agent.session)
    const stdin = makeStdin()
    ...
    ctx.reflect.get.mockImplementation((name: string) => (
      name === 'approvalRules.persistAllow'
        ? { persistAllowRule: async (req: unknown) => { persisted.push(req); return { tool: 'bash', pattern: 'echo', decision: 'allow', layer: 'project' } } }
        : undefined
    ))
    const owner = { id: app.sessionId ?? SessionId('approval-p') }
    const outcome = handler(
      { agent: { session: { id: owner.id } }, toolName: 'bash' },
      () => Promise.resolve('unavailable'),
    )
  ...
  it('p 无 approvalRules facet：告警进 scrollback，卡片保持挂起（可改选 y/n）', async () => {
    ...
    const handler = ctx.on.mock.calls.find(call => call[0] === 'approval/request')?.[1] as
      ...
      | undefined
    if (handler === undefined) throw new Error('approval/request handler not registered')
    const owner = { id: app.sessionId ?? SessionId('approval-p2') }
    const outcome = handler(
      { agent: { session: { id: owner.id } }, toolName: 'bash' },
      () => Promise.resolve('unavailable'),
    )
    stdin.emit('data', 'p')
    await new Promise(resolve => setImmediate(resolve))
    const warned = stdout.write.mock.calls.map(c => `${c[0]}`).join('')
    expect(warned).toContain('approval-rules 未装配')
    // 卡片未结算：随后 y 仍可放行
    stdin.emit('data', 'y')
    await expect(outcome).resolves.toBe('allowed-once')
    await app.dispose()
...
describe('TuiApp 运行中排队（对标 CC queue；↑ 取回）', () => {
  async function bootQueueApp() {
    ...
    const id = app.sessionId
    if (id === null) throw new Error('no active session')
    return { app, ctx, agent, stdout, stdin, id, emit: sessionEventBus(ctx) }
  }
  ...
  it('switchSession 丢弃排队并在旧会话视图回显条数', async () => {
    const { app, ctx, agent, stdout, id, emit } = await bootQueueApp()
    emit(id, { seq: 1, time: 1, type: 'turn/start', data: { turn: 1 } })
    app.handleSubmit('doomed message')
    await new Promise(resolve => setImmediate(resolve))
    stdout.write.mockClear()

    // 目标会话不可得（getSession undefined → mountSession 抛 unknown session）：
    // 丢弃与回显发生在挂载校验之前——警示行已进旧会话视图、队列已清空，
    // 其后 turn/end completed 也不再投递被丢弃的消息。
    ctx.sessions.get.mockReturnValue(undefined)
    await expect(app.switchSession(SessionId('queue-missing'))).rejects.toThrow('unknown session')
    const written = stdout.write.mock.calls.map(c => `${c[0]}`).join('')
    expect(written).toContain('丢弃 1 条未发送的排队消息')
  ...
  it('Ctrl+Enter 插队：cancel(keepInbox) 打断，idle 落定后走正常提交路径直发（调用序 cancel → followup）', async () => {
    ...
    expect(agent.cancel).toHaveBeenCalledWith({ kind: 'user' }, { keepInbox: true })
    expect(agent.followup).not.toHaveBeenCalled()
    // 落定 → whenIdle 兑现 → agent 已 idle → handleSubmit 直发
  ...
  it('Ctrl+Enter 空闲时不消费：草稿保留、无打断无发送', async () => {
    ...
    // 草稿仍在输入轨
    const written = stdout.write.mock.calls.map(c => `${c[0]}`).join('')
    expect(written).toContain('still here')
    await app.dispose()
  })
})
