...
import { color, hyperlink } from '../engine/ansi.js'
import type { RivetTheme } from '../theme.js'
import { hiddenLinesMarker } from './hidden-lines.js'
import { diffLineColor, isDiffContent } from './diff.js'
    ...
    if (text[i] === '[') {
      ...
      if (textEnd !== -1 && text[textEnd + 1] === '(') {
        ...
        if (urlEnd !== -1) {
          ...
          segments.push({ text: text.slice(i + 1, textEnd), underline: true, ...(href ? { href } : {}) })
          i = urlEnd + 1; continue
        }
      }
    }
...
export function parseBlocks(text: string): Block[] {
  ...
  while (i < lines.length) {
    const line = lines[i]
    if (line === undefined) break // unreachable: i < lines.length
    ...
    if (line.startsWith('```')) {
      ...
      blocks.push({ type: 'code', ...(language ? { language } : {}), content: codeLines.join('\n') })
      i++; continue
    }
    ...
    if (line.startsWith('$$') || line.startsWith('\\[')) {
      ...
      while (i < lines.length) {
        const l = lines[i]
        if (l === undefined || l.includes(closer)) break
        bodyLines.push(l); i++
      }
    ...
    if (line.startsWith('> ')) {
      ...
      while (i < lines.length) {
        const q = lines[i]
        if (q === undefined || !q.startsWith('> ')) break
        quoteLines.push(q.slice(2)); i++
      }
    ...
    if (/^(\s*[-*]\s|\s*\d+\.\s)/.test(line)) {
      ...
      while (i < lines.length) {
        const item = lines[i]
        if (item === undefined || !/^(\s*[-*]\s|\s*\d+\.\s)/.test(item)) break
        items.push(item.replace(/^\s*[-*]\s|\s*\d+\.\s/, '')); i++
      }
    ...
    if (line.includes('|') && i + 1 < lines.length && /^\|?[\s-:|]+\|?$/.test(lines[i + 1] ?? '')) {
      ...
      while (i < lines.length) {
        const t = lines[i]
        if (t === undefined || !t.includes('|')) break
        tableLines.push(t); i++
      }
    ...
    while (i < lines.length) {
      const p = lines[i]
      if (p === undefined || p.trim() === '' || p.startsWith('#') || p.startsWith('```') || p.startsWith('> ') || /^(\s*[-*]\s)/.test(p)) break
      paraLines.push(p); i++
    }
...
function formatSegment(seg: Segment, theme: RivetTheme): string {
  ...
  const opts = {
    ...
    ...(seg.dimmed !== undefined ? { dim: seg.dimmed } : {}),
  }
...
function formatCodeBlock(language: string | undefined, content: string, _columns: number, theme: RivetTheme): string[] {
  ...
  // Diff 块检测：diff/patch 语言标签直接命中；未标注语言时走 isDiffContent
  // 启发式（需 ≥2 信号或 hunk+± 行），避免普通代码里的 +/- 行被误染红绿。
  ...
  for (const line of visible) {
    // Diff 段：逐行套红绿（+ 绿 − 红，行分类与 formatDiff 一致）
    if (isDiffBlock) {
