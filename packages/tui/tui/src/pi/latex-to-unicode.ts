...
function bunColorShim(spec: string, format: 'css' | AnsiColorFormat): string | null
...
class LatexParser {
  ...
  #sqrt(style: FontStyle | null): string {
    ...
    if (env === 'array' || env === 'tabular' || env === 'array*' || env === 'tabular*' ||
        env === 'alignedat' || env === 'alignedat*' || env === 'alignat' || env === 'alignat*' ||
        env === 'gatheredat') {
      this.#optionalRawArgument()
      if (this.#s[this.#i] === '{') this.#rawArgument() // column spec / column count
    }
    let body = ''
    while (this.#i < this.#s.length) {
