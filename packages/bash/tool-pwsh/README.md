...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#huiliyi37dsh-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@huiliyi37/dsh-tool-pwsh-persistent`](../../pty/tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls on Windows (ConPTY) and POSIX hosts with pwsh.
