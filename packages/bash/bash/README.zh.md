...
| 包 | 职责 |
|---|---|
| `@huiliyi37/dsh-bash`（本包） | Service Definition：抽象服务 + 词汇类型 |
| `@huiliyi37/dsh-bash-local` | Service provider：本地子进程 |
| `@huiliyi37/dsh-bash-sandbox` | Service provider：沿用 `dsh-bash-local` 的机制，但通过 [`ctx.sandbox`](../../sandbox/sandbox/) 限制每次 spawn，并将拒绝报告为结果事实 |
| `@huiliyi37/dsh-tool-bash` | 基于 `ctx.bash`、面向模型的工具 schema |
