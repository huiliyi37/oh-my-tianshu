...
import type { WorkflowAgentInfo } from '@huiliyi37/dsh-workflow'
...
describe('dsh-tool-ralph over the real spawn and worker-thread stack', () => {
  it('uses distinct empty-seed children, shared cwd, and only the prior bounded handoff', async () => {
    ...
    const phases: string[] = []
    ctx.on('workflow/phase', (_run, title) => { if (typeof title === 'string') phases.push(title) })
    ctx.on('workflow/agent-start', (_run, child: WorkflowAgentInfo) => {
