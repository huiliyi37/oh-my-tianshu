...
import * as WorkflowInvariant from '@huiliyi37/dsh-workflow/invariant'
...
describe('workflow invariants', () => {
  it('accepts a complete workflow and child lifecycle', async () => {
    ...
    // TUI Wire 增强使 emit 的 phase payload 类型为 { title }；引擎侧发 string
    // 形状（invariant 钩子按引擎语义消费），显式断言到宽松派发签名。
