...
面向人类的 `/next-workflow <objective>` 命令运行一条固定的、由 harness 持有的相位机——INTENT → PLAN → CRITIQUE → IMPLEMENT → VERIFY → REVIEW，当 `planCandidates` 大于 1 时在 PLAN 与 CRITIQUE 之间插入可选的 SELECT 相位。编排由 harness 持有，模型只写内容。插件注册在 [`ctx.commands`](../../interaction/commands/README.md) 上，因此每个已装配的命令适配器（TUI、web）无需客户端改动即可发现并执行它。设计理由归 [intent-pipeline Agent Note](../../../.agents/notes/proposed/feature/2026-08-17-next-workflow-intent-pipeline.md) 所有。
...
| 输入 | 结果 |
|---|---|
| `/next-workflow [candidates] <objective>` | **后台启动**流水线并立即应答——对话界面不会在多分钟的运行上卡住。每个相位迁移都以追加的插件 notice 消息落进会话（`[next-workflow <run-id>] …`），包括最终汇总或失败；前导整数（1–5）覆盖本次运行的 `planCandidates`（`/next-workflow 3 …` = 三方案选优），不带则用 Config 缺省值。运行自身的结果对象携带完整汇总；verify 重试耗尽时以指明 `failed-verification` 的错误收尾。 |
