...
class StubProvider implements SubagentProvider {
  readonly capabilities: SubagentCapabilities = {
    outputSchema: true, depthLimit: true, toolFilter: true, persona: false, sandboxMode: false,
  }
...
describe('dsh-workflow-workerthread', () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('a child result REJECTION crosses back as a fatal AGENT_RESULT error (a broken provider is not a failed child)', async () => {
      ...
      const provider: SubagentProvider = {
        name: 'rejecting',
        capabilities: { outputSchema: true, depthLimit: true, toolFilter: true, persona: false, sandboxMode: false },
  ...
  describe('lifecycle: parse errors, cancellation, termination, disposal', () => {
    ...
    it('cancel() aborts in-flight children (signal AND cancel RPC) and settles the run cancelled', async () => {
      ...
      ctx.on('workflow/end', (_info: WorkflowRunInfo, result: WorkflowResultInfo) => { runEnds.push(result) })
  ...
  describe('worker death', () => {
    it('the first death signal closes admission to messages Node delivers before exit', async () => {
      ...
      ctx.on('workflow/phase', (_info, title) => {
        // The dsh engine narrates phases as a bare string; the TUI wire shape
        // carries `{ title }` — accept both, keep only the engine's string.
  ...
  describe('service surface', () => {
    it('run ids are unique and lifecycle meta is the run\'s borrowed immutable value', async () => {
      ...
      ctx.on('workflow/start', (info: WorkflowRunInfo) => { eventMeta = info })
