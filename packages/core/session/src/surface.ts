 ...
 *
 * @module @huiliyi37/dsh-session/surface
 */
...
export class SurfaceManager implements SessionSurface {
  ...
  /**
   * Reset all folded surface state (rewind): the log was truncated, so the
   * incremental projection must start over from the surviving prefix.
   */
  reset(): void {
