...
export class Session {
  ...
  truncate(atSeq: number): void {
    if (atSeq < -1 || atSeq >= this.log.length) {
      throw new Error(`session truncate: seq ${atSeq} out of range [${-1}, ${this.log.length - 1}]`)
    }
    this.log.splice(atSeq + 1)
    // Derived caches all reset: they folded against the pre-truncation log
    // and would otherwise project dropped events forever.
...
export class SessionStore extends Service {
  ...
  async flush(session: Session): Promise<boolean> {
    ...
    const failure = results.find((result): result is PromiseRejectedResult => { return result.status === 'rejected' })
