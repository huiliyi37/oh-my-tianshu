...
describe('Session', () => {
  it('truncate 截断事件日志并重置派生状态（surface/requestHeader/后续 seq 连续）', () => {
    const session = Session.create(SessionId('truncate-1'))
    ...
    // 触发派生状态折叠（surface/requestHeader 缓存）
    void session.surface.nodes
    void session.requestHeader()

    // 截断到 seq 1（保留 turn/start + user/message）
    session.truncate(1)

    expect(session.events).toHaveLength(2)
    expect(session.events[0]!.seq).toBe(0)
    expect(session.events[1]!.seq).toBe(1)
    // 派生状态重置：后续 append 的 seq 从截断后长度续起
    session.append('turn/end', { turn: 1, reason: { kind: 'blocked' } })
    expect(session.events).toHaveLength(3)
    expect(session.events[2]!.seq).toBe(2)
    // surface 同步回退：仅 user/message（seq 1）是 surface 事件，无残留投影
    expect(session.surface.nodes).toEqual([1])
  })

  it('truncate 边界：atSeq 为 -1 清空全部事件', () => {
    const session = Session.create(SessionId('truncate-clear'))
    session.append('turn/start', { turn: 1 })
    session.append('turn/end', { turn: 1, reason: { kind: 'blocked' } })
    session.truncate(-1)
  ...
  it('truncate 边界：atSeq 超界抛错（不静默截断全量）', () => {
    const session = Session.create(SessionId('truncate-oob'))
    session.append('turn/start', { turn: 1 })
    expect(() =>{  session.truncate(5) }).toThrow(/seq/)
    expect(session.events).toHaveLength(1) // 未变更
  })
