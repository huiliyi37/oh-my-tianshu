...
| Key | Default | Meaning |
|---|---|---|
| `includeHarnessIdentity` | `true` | Include the fixed `You are an AI agent powered by the Tianshu Harness SDK.` order-−100 opener. Set false only when a compatibility deployment owns the complete system prompt. |
...
- `PromptSection` — `{ name, order, text }`. Sections are concatenated in ascending `order`; equal orders break ties by name. Repository-owned placements live in the sparse `FIRST_PARTY_SECTION_ORDER` table (adjacent entries differ by ≥ 10, so new first-party sections insert without renumbering); external plugins may use any finite order.
