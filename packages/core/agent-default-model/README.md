...
The deployment default used when a front door creates an Agent that has no session-local model selection. `AgentDefaultModelService` provides `ctx.agentDefaultModel`; direct front doors such as `tianshu run` and Host-backed front doors such as ApiProxy read the same service instead of owning parallel provider/model defaults.
