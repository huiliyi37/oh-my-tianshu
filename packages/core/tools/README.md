...
- `ctx.tools.restrictableNames(scope?: ScopeKey): ReadonlySet<string>` Names the scope could address ignoring restrictions — the global layer plus every scope-chain layer, before any mask filters it. Face-arming readers use this pre-filter set: a restriction they installed must not hide the names they still owe.
...
- **`tools/pre-execute` deliberately cannot rewrite `exec.arguments`** — logged and rendered args would desync from what ran; rewrites resolve one phase earlier, and the shipped design is [the pre-tool-input-rewrite Agent Note](../../../.agents/notes/implemented/feature/2026-06-30-pre-tool-input-rewrite.md).
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@huiliyi37/dsh-timeout-policy` wrapper.
