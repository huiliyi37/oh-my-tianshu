...
import type { RpcRequest } from '@huiliyi37/dsh-host-apiproxy/api'
import { RpcId } from '@huiliyi37/dsh-host-apiproxy/api'
...
describe('session.search', () => {
  ...
  it('propagates cancellation through visible-session collection and stops cold-summary work', async () => {
    ...
    const list = vi.fn((signal?: AbortSignal) => {
      ...
      return Promise.resolve(cold.map(header => ({ header })))
    })
    let locateCalls = 0
    ctx.provide('sessionPersistence', {
      list,
