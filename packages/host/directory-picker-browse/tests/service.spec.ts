...
import type { DirectoryPickerBrowseCapability } from '@huiliyi37/dsh-host-directory-picker'
...
describe('BrowseDirectoryPicker', () => {
  ...
  it('rejects non-absolute paths instead of rebasing them under the process cwd', async () => {
    for (const relative of ['', 'projects', './projects', '..']) {
      ...
      const createFailure = await capability.createDirectory(relative, 'child').catch((error: unknown) => { return error })
