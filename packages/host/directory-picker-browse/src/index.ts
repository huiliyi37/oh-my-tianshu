 ...
 * @module @huiliyi37/dsh-host-directory-picker-browse
 */
...
} from '@huiliyi37/dsh-host-directory-picker'
import type {
...
function ancestryCrumbs(target: string): DirectoryEntry[] {
  ...
  for (;;) {
    ...
    crumbs.unshift({ name: parent !== current ? basename(current) : current, path: current, hidden: false })
