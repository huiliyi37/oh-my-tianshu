...
无。`/style` 切换只经由 settings seam 自身的提交路径写入；不新增任何会话事件。包不变量伴生入口（`@huiliyi37/dsh-output-style/invariant`）在每次 `system-prompt/change` 时断言每次组装至多一个 `output-style` section。

## System prompt

#### What the model sees

多出一个有序 section，其正文是当前预设的逐字文本：

```markdown
Answer directly and concisely. Lead with the result or decision, then only the supporting detail needed to act on it. …
```

三段正文是固定的产品文案，由测试逐字节钉住（`OUTPUT_STYLE_TEXTS`）。无论 zen 阶段的工具面如何，该 section 都会渲染——风格与工具暴露正交；不希望如此部署可以直接不挂载本包。
