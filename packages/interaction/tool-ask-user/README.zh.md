...
模型会看到生成的 [`ask_user_question` schema](../../../docs/tool-catalog.md#huiliyi37dsh-tool-ask-user)，其中包含问题 id、提示语、标题、选项和多选标志。
...
- **待处理问题会阻塞工具调用，直至用户作答**：该工具未声明 `timeout-guard` 预算；取消仅沿用当前轮次的 `exec.signal`。
