...
import * as PermissionInvariant from '@huiliyi37/dsh-permission/invariant'
...
describe('permission invariants', () => {
  ...
  it('rejects an unknown preset already present on late registration', async () => {
    ...
    await expect(ctx.plugin(PermissionInvariant).then(() => { return undefined })).rejects.toThrow(/unknown preset "missing"/)
  })
})
