...
与通道无关的审批 seam。`ctx.approval.request(req)` 返回 `allowed-once`、`allowed-always`、`rejected`、`cancelled` 或 `unavailable`；应答者缺失或失败时会以拒绝方式关闭，两种授权都只作用于所请求的操作——`allowed-always` 额外记录本次请求由持续授权（持久规则或会话级总是允许）结算，保持审计轨迹诚实。确切事件签名见 [approval.md](../../../docs/subsystems/approval.md#cordis-surface) 的生成区块。
...
- **此处不承载授权存储**：`allowed-always` 是审计来源标记而非可撤销授权——持续行为存活在结算它的应答者里（approval-rules 的持久文件或 TUI 的会话开关）；会话策略只有 `ask`／`never`。
