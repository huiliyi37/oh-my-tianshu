...
| `summarizationProvider` | 否（默认 `''`） | 与 `summarizationModel` 一起设置；可选 `ctx.modelRoles` 服务中的 `secondary` 角色 pin 优先于非空对，空对会解析为最新已记录请求目标，再回退到 `AgentOptions` 对。 |
| `summarizationModel` | 否（默认 `''`） | 与 `summarizationProvider` 一起设置；可选 `ctx.modelRoles` 服务中的 `secondary` 角色 pin 优先于非空对，空对会解析为最新已记录请求目标，再回退到 `AgentOptions` 对。 |
