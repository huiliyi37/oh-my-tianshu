...
| `dsh-compact`, `dsh-hook-protocol`, `dsh-sandbox-policy` | 持久压缩（compaction）与钩子配对、压缩元数据和沙箱 mode 词汇。 |
| `dsh-fs`, `dsh-subagent`, `dsh-workflow` | 文件系统事件身份、提供方/子级配对和工作流/agent 生命周期身份。 |
| `dsh-goal`, `dsh-goal-session` | 持久 goal 来源/内容一致性、修订和生命周期转换、时间戳、依次获准的 Round 和重建的继续提示词。 |
...
ctx.plugin(InvariantService, {
  ...
  package_blocklist: ['^@huiliyi37/dsh-agent-loop$'],
})
