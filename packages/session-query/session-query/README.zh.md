...
该包没有提供方协调器、回退实现或独立具体插件。具体服务后端继承已实现的读取、过滤和跟踪，同时负责全文观察、对账、排名、游标世代和查询执行；第一个实现是 [`@huiliyi37/dsh-session-query-sqlite`](../session-query-sqlite/README.md)。
