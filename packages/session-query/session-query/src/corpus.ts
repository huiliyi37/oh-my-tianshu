...
import type { Session, SessionEvent, SessionHeader, SessionId } from '@huiliyi37/dsh-session'
import SessionPersistence, { SessionPersistenceCorruptionError } from '@huiliyi37/dsh-session-persistence'
...
export class SessionCorpus {
  ...
  async projectMany<Value>(
    ...
    const settlements = await Promise.allSettled(
      Array.from({ length: workerCount }, () => { return worker() }),
    )
    if (signal?.aborted) signal.throwIfAborted()
...
async function listPersisted(
  ...
  try {
    return (await persistence.list(signal)).map(entry => entry.header)
  } catch (error: unknown) {
    if (signal?.aborted) signal.throwIfAborted()
