 ...
 * `ctx.llm`.
 * @module @huiliyi37/dsh-web-search-perplexity/provider
 */
...
const USER_AGENT = 'tianshu-harness/0.2.1'
...
export class PerplexitySearchProvider implements WebSearchProvider {
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (!response.ok) {
      ...
      try {
        const parsed: PerplexityError = await response.json() as PerplexityError
