 ...
 */
import type { SlashCandidate, SlashSource } from '../types.ts'
...
export function seedGroups(
  state: MenuState,
  sources: readonly Pick<SlashSource, 'name' | 'showGroupTitle'>[],
): MenuState {
  return {
    ...state,
...
export const menuReduce: MenuReduce = (state, ev) => {
  switch (ev.type) {
    ...
    case 'source-settled': {
      ...
      const items: readonly SlashCandidate[] = ev.items ?? []
      const groups = state.groups.map((g, i) =>
        i !== idx ? g : { ...g, status: 'ready' as const, items })
