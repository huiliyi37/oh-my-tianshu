...
import type { SessionId, SessionSummary } from '@huiliyi37/dsh-client-connection/client'
...
describe('flattenLineage', () => {
  ...
  it('fails soft on a two-node cycle: all entries emitted, warn fired, no hang', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => { return undefined })
    try {
