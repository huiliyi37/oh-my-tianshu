...
import type { SessionId, WorkspaceId } from '@huiliyi37/dsh-client-runtime/client'
...
describe('workspace browser rows', () => {
  ...
  it('renders and opens a selected running Session row', () => {
    const node: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, corrupt: false, updatedAt: 0,
    }
    const onOpen = vi.fn()
    render(
  ...
  it('P2④ stage 3: a corrupt session row renders the unrecoverable badge and dims its title', () => {
    const node: SessionNode = {
      id: sid('corrupt'), title: 'Broken', blank: false, running: false,
      runningSubagentCount: 0, completed: false, corrupt: true, updatedAt: 0,
    }
    render(
    ...
    expect(screen.getByText('Broken').className).toContain('titleCorrupt')
  })
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, corrupt: false, updatedAt: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, corrupt: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, corrupt: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, corrupt: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false, corrupt: false, updatedAt: 0,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, corrupt: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
