...
import type { ISession, SessionId, WorkspaceId } from '@huiliyi37/dsh-client-runtime/client'
...
import { apply, inject } from '@huiliyi37/dsh-client-ui-workspace/client'
...
describe('session rename through the assembled browser', () => {
  it('renames via the row menu: binding.session.rename fires, the dialog closes, the row re-labels from the list', async () => {
    ...
    const input: HTMLInputElement = await view.findByLabelText('会话名称') as HTMLInputElement
