...
Layering: the pure core sits at the top level of `src/client` beside the shell — `detect.ts`, `menu.ts`, `core-contract.ts`, zero React/DOM/cordis; `service.ts` is the shell wiring the core to the menu snapshot store, the per-hit candidate fetch (generation-gated, `AbortSignal`-superseded, failed sources drop silently with a console record), and the three pick paths. `src/types.ts` and the two `contract.ts` files are the frozen cross-package contract; changes require main-thread arbitration.
