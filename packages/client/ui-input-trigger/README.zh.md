# @huiliyi37/dsh-client-ui-input-trigger
...
分层：纯内核与壳层并排在 `src/client` 顶层——`detect.ts`、`menu.ts`、`core-contract.ts`，零 React／DOM／cordis；`service.ts` 是壳层，把内核接到菜单快照 store、逐 hit 候选拉取（以 generation 把关、后继请求经 `AbortSignal` 取代旧请求、失败的 source 静默丢弃并留一条 console 记录）和三条 pick 路径上。`src/types.ts` 与两个 `contract.ts` 文件是冻结的跨包约定；变更需经主线程仲裁。
