 ...
 * @module
 */
import { abbreviateHomePath } from '@huiliyi37/dsh-client-runtime/client'
import type { ReadBlockLine, ReadBlockProps } from '@huiliyi37/dsh-client-ui-primitives'
...
export function readCardModel(block: ToolCallBlock, sessionCwd?: string, home?: string): ReadCardModel | null {
  ...
  const lines: Array<ReadBlockLine> = result.lines.map(line => ({ number: line.number, text: line.text }))
  return {
