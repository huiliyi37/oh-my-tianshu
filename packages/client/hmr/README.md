...
Hot reload for script-loaded client plugins. A static-arrival entry composed only into `--dev` graphs (`oh-my-tianshu web --dev`); production graphs omit the row, so the shell-bundled code stays inert.
