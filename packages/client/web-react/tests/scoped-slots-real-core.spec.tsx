...
import { SlotCore, type PropsRenderSlots, type SlotRendererHost } from '@huiliyi37/dsh-client-ui-slots'
import { createSlotRenderer, StaleAuthorizationError } from '@huiliyi37/dsh-client-web-react'
...
describe('createSlotRenderer over the real SlotCore', () => {
  ...
  it('passes owner props through and keeps sibling entries() references stable across mutations', async () => {
    ...
    core.register({ name: 'spec.single' }, ({ label }: { label?: string }) => { return <b>{label}</b> })
    const view = render(<>{createSlotRenderer().renderRoot(hostOver(core), {})}</>)
