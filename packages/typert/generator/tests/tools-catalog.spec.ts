...
import type { TypertContribution } from '@huiliyi37/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@huiliyi37/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 60_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@huiliyi37/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@huiliyi37/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@huiliyi37/dsh-tools', 'host')).toBeUndefined()
  })
})
