...
} from '@huiliyi37/dsh-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@huiliyi37/dsh-tools', 'ToolInput')).toBe('@huiliyi37/dsh-tools#ToolInput')
    expect(typertPackageKey('@huiliyi37/dsh-tools', 'host')).toBe('@huiliyi37/dsh-tools#host')
    expect(ctx.typert.get('@huiliyi37/dsh-tools#ToolInput')).toMatchObject({
      package: '@huiliyi37/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@huiliyi37/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@huiliyi37/dsh-tools', 'host')).toMatchObject({
      key: '@huiliyi37/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@huiliyi37/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@huiliyi37/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@huiliyi37/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@huiliyi37/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('rejects malformed contribution identities and filters both registry views', async () => {
    ...
    expect(() => { return ctx.typert.register({ ...toolsContribution(), package: '' }) })
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@huiliyi37/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@huiliyi37/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@huiliyi37/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@huiliyi37/dsh-tools#ToolInput')).not.toBe(projected)
  })
