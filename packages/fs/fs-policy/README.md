...
// Load it alongside a ctx.fs provider (e.g. @huiliyi37/dsh-fs-local) and the
// @huiliyi37/dsh-tool-fs tools; the tools dispatch the fs/* events this plugin
...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@huiliyi37/dsh-tool-fs` | model-facing schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@huiliyi37/dsh-fs-policy` (this) | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@huiliyi37/dsh-fs` | `ctx.fs`: text IO + atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@huiliyi37/dsh-fs-local` | local implementation of `ctx.fs` |

## How the gate participates

Three `fs/*` events (declared by `@huiliyi37/dsh-fs`, dispatched by `@huiliyi37/dsh-tool-fs`):
...
Because the plugin influences the world only through events, removing it does not break `@huiliyi37/dsh-tool-fs` at a service-injection boundary: the tool falls through to the bare `ctx.fs` provider (unconditional write/edit, no observed-state). Loading it back layers the policy on. That graceful add/remove is the whole point of the event gate over a mandatory method service.
