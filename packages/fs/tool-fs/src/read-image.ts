 ...
 * 天枢分叉适配（documented divergence）：本仓 ContentBlock 的图片块为 dataUrl
 * 形态（@huiliyi37/dsh-llm types），不携带上游的 `attachment` 引用。image 块改以
 * 富字段输出持久溯源（attachmentId/mediaType/dims/name/originalDimensions），
 * dataUrl 由持久存储读回字节现编——持久去重与维度报告双保留；上游
 * WireFileContentPart 引用形态待 Files 波次后续决策。
 * @module @huiliyi37/dsh-tool-fs/src/read-image
 */

import { basename, extname } from 'node:path'
import type { Context } from '@huiliyi37/cordis'
import { AttachmentError, AttachmentId } from '@huiliyi37/dsh-attachment'
import type { ImageAttachmentRef, ImageMediaType } from '@huiliyi37/dsh-attachment'
import type { ContentBlock } from '@huiliyi37/dsh-llm'
import { defineTool } from '@huiliyi37/dsh-tools'
import type { GenericCallView, ToolExecution } from '@huiliyi37/dsh-tools'
...
export async function assertImageCapableRoute(ctx: Context, exec: ToolExecution, requestedPath: string): Promise<void> {
  ...
  // 两种能力声明并兼容：本仓目录 supportsVision 布尔；上游 Files 波次的
  // inputModalities 数组。缺省视为纯文本，从严拒绝。
  const modalityOk = active.supportsVision === true
    || (active as { inputModalities?: readonly string[] }).inputModalities?.includes('image') === true
  if (!modalityOk) {
 ...
 */
function imageReadContent(value: ImageReadValue & { dataUrl: string }): ContentBlock[] {
  return [
    { type: 'text', text: formatImageReadOutput(value.path, value.image) },
    {
      // 分叉形态：序列化只消费 dataUrl；其余字段是持久溯源元数据（非 wire 契约）。
      type: 'image',
...
export function applyReadImageTool(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => imageReadContent(value as ImageReadValue & { dataUrl: string }),
    },
    ...
    async execute(args, exec) {
      ...
      const dataUrl = `data:${ref.mediaType};base64,${Buffer.from(stored.data).toString('base64')}`
      const value: ImageReadValue & { dataUrl: string } = {
        dataUrl,
