 ...
 * in hook order.
 * @module @huiliyi37/dsh-hook-protocol/merge
 */
...
export function mergeHookOutputs(outputs: HookOutput[]): MergedHookOutcome {
  ...
  for (const out of outputs) {
    ...
    if (out.additionalContext !== undefined && out.additionalContext.length > 0) additionalContext.push(out.additionalContext)
