...
`supportsVision` declares whether a model accepts image input. It resolves to pi-ai's `Model.input` modalities and is surfaced as `supportsVision` on `ctx.llm.listModels`/`resolveModelInfo`, which is what the composer's direct-send gating, the vision bridge's auto-pick, and `ask_image` consult. Omitting the field keeps the installed entry's modalities — a hand-declared model has none and is text-only; `true` declares image input, forcing it onto a catalog model shipped text-only; `false` strips it from one the catalog ships multimodal, for a gateway that cannot serve images. A request carrying images to a text-only model fails with `UNSUPPORTED_CONTENT` before credentials or network I/O: pi-ai itself would only downgrade the images to text placeholders, which would look like the model having seen them.
...
- **The layered merge has no delete for dict keys** — the settings seam merges the composition `base` and the user layer per key, recursively, so a `reasoningEfforts` level, `modelOverrides` entry, or `compat` field the base declares cannot be removed by the user layer, only overridden — and for `reasoningEfforts` absence *is* the meaning ("not offered"), so a base-declared level stays offered. This triggers when a `cordis.yml` entry config declares per-model reasoning fields for the same model the user layer edits — the shipped composition's one configured route (`openrouter`) does declare them, so editing that model is better done by restating its `models` list, which replaces wholesale and is the in-band escape.
