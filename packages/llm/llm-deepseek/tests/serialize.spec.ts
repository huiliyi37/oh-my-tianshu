...
describe('serializeMessages', () => {
  ...
  it('maps user images to multimodal content parts (image_url)', () => {
  ...
  it('maps image-only user message to a single image_url part', () => {
    const img = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='
    const wire = serializeMessages([
      createUserMessage({
        content: [{ type: 'image', dataUrl: img }],
        source: { kind: 'plugin', plugin: 'test' },
      }),
    ])
    expect(wire).toEqual([{
  ...
  it('keeps images alongside tool results (image + tool-result, no text)', () => {
    const img = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='
    const wire = serializeMessages([
      createUserMessage({
        content: [
...
describe('serializeRequest', () => {
  const history: Array<Message> = [createUserMessage({
