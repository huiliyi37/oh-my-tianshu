/** Content-block structure helpers. @module @huiliyi37/dsh-llm/content */

import type { ContentBlock, ImageBlock } from './types.ts'
import type { Message } from './message.ts'
import type { ImageAttachmentRef, RequestImageAttachment } from '@huiliyi37/dsh-attachment'
...
/**
 * Model-facing stand-in for an image a text-only route cannot accept. Local
 * image blocks carry inline data URLs without a durable identity, so the
 * placeholder is the bare form of {@link textOnlyImageText}.
 */
export const TEXT_ONLY_IMAGE_TEXT = '[image omitted because this model accepts text only]'
...
/**
 * Base64 payload length of one data-URL image block — the wire cost the
 * request bound cares about. The payload starts after the first comma; a
 * malformed URL conservatively counts its whole length.
 */
function imagePayloadLength(dataUrl: string): number {
  const comma = dataUrl.indexOf(',')
  return comma < 0 ? dataUrl.length : dataUrl.length - comma - 1
}

/**
 * Raw byte length one data-URL image block decodes to. Canonical payloads
 * round-trip exactly through base64 accounting; a malformed or padded tail
 * over-counts by at most two bytes, erring toward earlier offloading.
 */
function imageRawLength(block: ImageBlock): number {
  return Math.floor(imagePayloadLength(block.dataUrl) / 4) * 3
}

/** Base64 payload length of a raw byte count. */
...
export interface RequestImageOffloadPolicy {
  ...
  /** Resolve one image's encoded request byte length; omission accounts the inline data-URL representation. */
  byteLength?: (block: ImageBlock) => number
}
...
function collectImageLengths(
  ...
  for (const block of blocks) {
    if (block.type === 'image') {
      if (policy.byteLength === undefined) {
        // The default accounts the inline representation itself: exact base64
        // payload characters, or the decoded raw estimate under 'raw'.
        lengths.push(policy.representation === 'base64' ? imagePayloadLength(block.dataUrl) : imageRawLength(block))
      } else {
        const bytes = policy.byteLength(block)
...
function replaceImagesForTextModel(blocks: readonly ContentBlock[]): ContentBlock[] {
  ...
  for (const [index, block] of blocks.entries()) {
    if (block.type === 'image') {
      next ??= blocks.slice(0, index)
      next.push({ type: 'text', text: TEXT_ONLY_IMAGE_TEXT })
      continue
    }
    if (block.type === 'tool-result') {
 ...
 * is deterministic from durable message order and data-URL length; a provider
 * can serialize the returned messages without reading omitted bytes.
