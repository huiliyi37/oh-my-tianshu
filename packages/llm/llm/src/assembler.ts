 ...
 *
 * @module @huiliyi37/dsh-llm/assembler
 */
...
export class BlockAssembler {
  ...
  private assembled(): { blocks: ContentBlock[]; replay: ReplayEnvelope | undefined } {
    ...
    const kept = this.finish.kind === 'max-tokens'
      ...
      : undefined
    const blocks = kept === undefined ? all : all.filter((_block, position) => kept[position] === true)
    ...
    return {
      ...
      replay: kept === undefined || blocks.length === all.length
        ? envelope
        : { response: envelope.response, blocks: envelope.blocks.filter((_entry, position) => kept[position] === true) },
    }
  }
