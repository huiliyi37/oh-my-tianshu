...
/**
 * Derive the internal projection modality vocabulary from the catalog
 * `supportsVision` boolean. The fork keeps `supportsVision` on adapter
 * catalogs and configs; this one catalog-to-internal derivation feeds the
 * projection surfaces (prepared calls and text-only image projection), which
 * speak upstream's `inputModalities`.
 * @param info - validated exact-model metadata carrying the catalog capability.
 * @returns `['text', 'image']` for a declared vision capability, otherwise `['text']`.
 */
function inputModalitiesOf(info: LlmResolvedModelInfo): readonly ModelModality[] {
  return info.supportsVision === true ? ['text', 'image'] : ['text']
}
...
export class LlmService extends Service {
  ...
  async listModels(provider: string): Promise<LlmModelInfo[]> {
    ...
    return models.map((model) => {
      if (
        ...
        || (model.supportsVision !== undefined && typeof model.supportsVision !== 'boolean')
        || seen.has(model.id)
      ) {
  ...
  private normalizeModelInfo(
    ...
    if (
      ...
      || (resolved.supportsVision !== undefined && typeof resolved.supportsVision !== 'boolean')
    ) {
      throw new LlmError(
  ...
  async prepareCall(config: LlmCallConfig, signal?: AbortSignal): Promise<PreparedLlmCall> {
    ...
    return Object.freeze({
      ...
      inputModalities: Object.freeze([...inputModalitiesOf(modelInfo)]),
      stream: (options: GenerateOptions): AsyncIterable<StreamChunk> => {
        if (dispatched) {
  ...
  private async * adapterStream(
    ...
    try {
      ...
      // A text-only route receives deterministic placeholders instead of image
      // blocks. The projection happens after the loop-visible waterfall, so
      // listeners still observe the durable image history.
      const projectedOptions = !inputModalitiesOf(modelInfo).includes('image')
