...
describe('LlmService', () => {
  ...
  it('passes through the supportsVision capability from advisory and exact model metadata', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmService)
    ...
    await expect(ctx.llm.listModels('catalog')).resolves.toEqual([{
      provider: 'catalog', id: 'vl', name: 'VL', description: 'sees images', supportsVision: true,
    }])
    // exact-model resolution 也透传（CatalogAdapter 默认按 name 兜底；这里用一个自定义 adapter）
    const exact = new class extends ScriptedAdapter {
      override resolveModel(): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({
          provider: 'exact', id: 'vl', name: 'VL', description: 'sees images', supportsVision: true,
        })
      }
    }(SCRIPT)
    ctx.llm.registerAdapter(['exact'], exact)
    await expect(ctx.llm.resolveModelInfo('exact', 'vl')).resolves.toMatchObject({ supportsVision: true })
  })
  ...
  it.each([
    ...
    [{ provider: 'route', id: 'model', name: 'Model', supportsVision: 1 }, 'non-boolean supportsVision'],
  ...
  it.each([
    ...
    [{ provider: 'route', id: 'm', name: 'M', supportsVision: 1 }, 'non-boolean supportsVision'],
