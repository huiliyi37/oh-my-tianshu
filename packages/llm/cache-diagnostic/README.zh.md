...
- `hitRate(session)` — 整个会话的累计缓存命中率。
- `recentHitRate(session, lastN)` — 最近 N 轮的同一命中率。

命中率是计费输入中的缓存占比：`cacheRead / (inputTokens + cacheRead + cacheWrite)`。`TokenUsage` 各项计数互不相交（`inputTokens` 仅是未缓存部分），因此从不报写 token 的 provider（DeepSeek）也能得到精确命中率，而不是退化为 100%。
...
`diagnoseCacheMiss` 按下述顺序分类最新一轮：首轮（尚无可缓存内容）→ provider 从不报缓存计数（无法度量）→ 命中率 ≥ 0.8（无需解释）→ 前缀漂移 → 压缩 → **前缀截断** → 缓存驱逐 → 正常增长。漂移与压缩只在其事件落入最新一轮的度量窗口（上一轮最后一次 usage 之后）时才参与归因，过期信号不会误标后续健康轮次。
...
间接体现，经 TUI 状态栏等消费方；服务本身不新增任何提示词、消息、schema、工具或模型调用。

#### KV Cache 影响

无直接失效；本包度量缓存效果，从不改动请求。
...
- **0.8 / 0.4 阈值是诊断启发式常数** — 固定常数而非部署可调项：它们分类的是健康信号而非行为，上游遥测发货的也是这两个值。
