...
- `hitRate(session)` — cumulative cache hit rate: `cacheRead / inputTokens` over the whole session.
- `recentHitRate(session, lastN)` — the same rate over the last N turns.

Hit rates use **total input tokens** as the denominator rather than `cacheRead + cacheWrite`, so a provider that omits write tokens cannot degenerate the rate to 100%.
...
`diagnoseCacheMiss` (`src/diagnose.ts`) classifies the latest turn in this order: no cache counters → first turn → hit rate ≥ 0.8 (nothing to explain) → prefix drift → compaction → **prefix truncation** → cache eviction → ordinary growth.
