...
- **与 wire 截断精确互补（同源门控）**：锚点提取用同一 N 同一 tokenizer（`truncateCutStart` / `extractExcludedClaims` 来自 `dsh-llm-deepseek`）；N **与总开关 enabled 都**从 `llm-deepseek` 的 settings 命名空间读取（同源无漂移）——`spark.enabled=false`（含缺省）时 wire 不截断，锚点同样不注入（fail-closed）；无 settings 服务时保守不注入。
...
- N 档位与总开关依赖 `llm-deepseek` settings 命名空间；若未来 llm-deepseek 改名/摘除，需同步本插件读取点。
