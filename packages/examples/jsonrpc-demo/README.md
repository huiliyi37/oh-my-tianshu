...
A config without `dsh-sdk-server` is valid and serves nothing; the bin does not designate a server plugin.
...
- **The bin cannot prove that the config serves JSON-RPC** — a valid config with no `dsh-sdk-server` entry boots successfully and serves nothing.
