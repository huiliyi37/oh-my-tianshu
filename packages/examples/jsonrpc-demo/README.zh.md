...
不含 `dsh-sdk-server` 的配置仍然有效，只是不提供任何服务；bin 不会指定服务器插件。
...
- **bin 无法证明配置提供 JSON-RPC 服务**：不含 `dsh-sdk-server` 条目的有效配置也能成功启动，但不会提供任何服务。
