...
/**
 * Keyless built-artifact smoke: plain Node imports `@huiliyi37/dsh-lsp` and
 * `@huiliyi37/dsh-lsp-local` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@huiliyi37/dsh-lsp')
      const LspLocal = await import('@huiliyi37/dsh-lsp-local')
      const { default: LocalFileSystem } = await import('@huiliyi37/dsh-fs-local')
