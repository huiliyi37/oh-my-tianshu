...
import type { PtySendRequest, PtyWaitReason } from '@huiliyi37/dsh-pty'
import { LocalPtyBackend, PWSH_PROMPT_SETUP } from '@huiliyi37/dsh-pty-local'
import { ENCODING_PREAMBLE } from '@huiliyi37/dsh-pwsh-local'
...
describe('LocalPtyBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect((ctx.sandbox as RecordingSandbox).calls).toEqual([{
      argv: ['/bin/bash', '-i'],
      policy: { mode: 'workspace-write', workspaceRoot: resolve('/workspace') },
    }])
  })
  ...
  it('resolves session mode and root together before wrapping the shell', async () => {
    ...
    expect((ctx.sandbox as RecordingSandbox).calls).toEqual([{
      argv: ['/bin/bash', '-i'],
      policy: { mode: 'workspace-write', workspaceRoot: resolve('/session-workspace') },
    }])
  })
