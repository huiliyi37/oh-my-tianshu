...
import ToolRegistry, { type JsonValue } from '@huiliyi37/dsh-tools'
import { publicToolName, syncTools, type ToolBridgeOptions } from '@huiliyi37/dsh-mcp-client/src/tools.ts'
import { createTransport } from '@huiliyi37/dsh-mcp-client/src/transport.ts'
import type { Config } from '@huiliyi37/dsh-mcp-client'
...
describe('tool execution', () => {
  ...
  it('preserves primitive JSON MCP blocks while Native rendering marks them unsupported', async () => {
    const blocks = [42, null, ['nested']] satisfies Array<JsonValue>
    const client = createMockClient(
