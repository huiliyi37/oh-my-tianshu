...
- `ctx.skills.registerProvider(create): () => void` Calls a synchronous provider factory with `{ signal, invalidate }`, then registers its readonly result by `provider.name` unique within its registration layer: the calling scope decides the layer, so two live preset standing compositions may each run a same-named provider (the catalog view resolves the nearest scope's entry, global last) while a duplicate inside one layer still throws. `runtime` is reserved, and failed registration aborts the signal. The exact Cordis disposer unregisters the provider, aborts the signal, and preserves ordered composite teardown.
