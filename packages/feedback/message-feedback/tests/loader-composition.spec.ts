...
import Storage from '@huiliyi37/dsh-storage'
import * as StorageDomain from '@huiliyi37/dsh-storage-domain'
import * as StorageJson from '@huiliyi37/dsh-storage-json'
import { remoteMethods } from '@huiliyi37/dsh-type-meta'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@huiliyi37/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@huiliyi37/dsh-storage', Storage],
    ['@huiliyi37/dsh-storage-json', StorageJson],
    ['@huiliyi37/dsh-storage-domain', StorageDomain],
    ['@huiliyi37/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@huiliyi37/dsh-storage'",
      "- name: '@huiliyi37/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@huiliyi37/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@huiliyi37/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
