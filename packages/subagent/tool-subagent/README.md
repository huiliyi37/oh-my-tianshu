...
A foreground call passes the execution signal through startup and execution, awaits `run.result`, and always awaits `run.dispose()` before returning. Optional `runBudget` config transfers the same step and wall-clock bound to every foreground and one-shot background child; omission leaves those runs provider-managed. Only `completed` returns the canonical `{ kind: 'foreground', runId, output: JsonValue[] }`, rendered as the same final text; abort, refusal, token limit, and other failures become errored tool results without partial output. If result collection and disposal both reject, the errored result preserves both diagnostics.
...
| `agentOptions` | Provider-specific child `provider`, `model`, and positive `maxTokens`; the in-process provider treats explicit values as overrides of the `subagent` role pin and the inherited parent options. Requires the provider's `agentOptions` capability — the service rejects the request and this plugin's mount fails loud on a provider that would silently ignore the route. |
| `modelSelectionSettings` | Let the model select the child LLM route per call (`provider`+`model` together, optional `reasoning_effort`), gated by the Host `subagent-model-selection` settings section (default off; exact-route allowlist). Compose `@huiliyi37/dsh-tool-subagent/model-selection-settings` in the Host scope; each session records its decision once as the log-only `subagent/model-selection-policy` event at first delegation, and the fixed-schema `list_subagent_models` tool discovers authorized routes from the live LLM directory. The shipped `subagent_fork` instance deliberately keeps selection off to preserve inherited-prefix KV Cache reuse. |
...
| `runBudget` | Optional `{ maxSteps, timeoutMs }` bound for each one-shot foreground/background child; requires the provider's `runBudget` capability and fails the mount without it. `timeoutMs` must fit the non-clamping Node timer range. Omission leaves runtime bounds provider-managed. |
