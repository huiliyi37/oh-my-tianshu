...
import { createUserMessage, CallId, type ContentBlock, type GenerateOptions } from '@huiliyi37/dsh-llm'
...
import type { Config as ToolConfig, ObjectJsonSchema } from '@huiliyi37/dsh-tools'
import { defineContentToolFixture, RUN_CODE_NAME } from '@huiliyi37/dsh-tools'
...
async function setup(script: Script, options: SetupOptions = {}) {
  ...
  const disposeProvider = ctx.subagents.registerProvider({
    name: 'spawn',
    capabilities: { agentOptions: true, outputSchema: true, depthLimit: true, toolFilter: false, persona: false, sandboxMode: false, runBudget: false },
