...
import { carrierKeyOf } from '@huiliyi37/dsh-scope'
...
describe('SubagentService', () => {
  ...
  it('publishes lifecycle only after async provider start and keeps parent scope', async () => {
    ...
    ctx.on('subagent/start', function (info) { events.push('start'); keys.push(carrierKeyOf(this)); if ('runId' in info) runIds.push(info.runId) })
    ctx.on('subagent/end', function (info) { events.push('end'); keys.push(carrierKeyOf(this)); if ('runId' in info) runIds.push(info.runId) })
  ...
  it('mints distinct lifecycle identities when provider and child ids repeat', async () => {
    ...
    ctx.on('subagent/start', (info) => {
      if ('runId' in info) runIds.push(info.runId)
    })
  ...
  it('contains synchronous and asynchronous lifecycle observer failures', async () => {
    ...
    // oxlint-disable-next-line typescript/no-misused-promises -- deliberate: this case pins the rejected-thenable containment path.
