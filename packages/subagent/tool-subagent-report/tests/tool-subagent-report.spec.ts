...
import { CallId, LlmAdapter, createUserMessage } from '@huiliyi37/dsh-llm'
...
import * as control from '@huiliyi37/dsh-tool-subagent-control'
...
describe('dsh-tool-subagent-report', () => {
  ...
  it('keeps a next-step report parked in a busy parent across child settlement', async () => {
    const { ctx, parent, adapter } = await setup({ config: { reportDelivery: 'next-step' } })
    await startHeldParentTurn(parent, adapter)
    const { started, child } = await startChild(ctx, parent)
    ...
    expect(parent.inbox.nextStep.map(message => message.source.kind)).toEqual(['subagent-report'])
