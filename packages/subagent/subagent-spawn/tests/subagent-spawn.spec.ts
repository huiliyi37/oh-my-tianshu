...
import { Context, symbols, type EffectMeta } from '@huiliyi37/cordis'
...
import SubagentService, { type SubagentStartRequest } from '@huiliyi37/dsh-subagent'
...
describe('dsh-subagent-spawn', () => {
  ...
  it('emits subagent/start only after the fresh child is published', async () => {
    ...
    ctx.on('subagent/start', (info) => {
      if ('provider' in info && info.provider === 'spawn') childAtStart = ctx.agents.get(info.id)
    })
  ...
  it('advertises every start-time capability (depthLimit, outputSchema, toolFilter, persona)', async () => {
    ...
    const provider = ctx.subagents.getProvider('spawn')!
    expect(provider.capabilities).toEqual({ outputSchema: true, depthLimit: true, toolFilter: true, persona: true, sandboxMode: true })
  })
