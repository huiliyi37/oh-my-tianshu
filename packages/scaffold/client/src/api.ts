 ...
 *
 * @module @huiliyi37/dsh-sdk-client/api
 */
...
/**
 * Reusable SDK for running Tianshu Harness agent turns in a runtime
...
export function finalResponse(events: SessionEvent[]): string {
  for (let index = events.length - 1; index >= 0; index--) {
    ...
    return event.data.message.content
      .filter((block): block is ContentBlock & { type: 'text' } => { return block.type === 'text' })
