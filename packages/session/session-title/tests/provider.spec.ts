...
import LlmService, { createUserMessage, deepFreeze, markAgentLoopRequest  } from '@huiliyi37/dsh-llm'
...
describe('SessionTitleService provider lifecycle', () => {
  ...
  // fb34fb6b 中间态曾确定性失败（见 #5），但最终 squash 进 main 的实现
  // （onMainRequest 经 llm/stream prepend 挂点 + requestHeader fold 读取）已满足
  // 本场景：在引入 it.skip 的 3505cc2 与当前 main 上均确定性通过（42/42），翻回。
  it('runs an all-messages revision when the next main request reuses its logged header', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmService)
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionTitleService, CONFIG)
    const requests: Array<SessionTitleProviderRequest> = []
    ctx.sessionTitle.register({
