 ...
 * @module @huiliyi37/dsh-session-persistence-jsonl
 */
...
export class SessionPersistenceJsonl extends SessionPersistence implements PersistenceBackend<JsonlTornMarker> {
  ...
  truncateStored(id: SessionId, atSeq: number): Promise<void> {
    return this.coordinator.truncateStored(id, atSeq)
  }
  ...
  /**
   * Durably delete stored events with `seq > atSeq` (rewind): rewrite the log
   * with the header plus the surviving prefix, then fsync. O(n) full rewrite —
   * acceptable because rewind is a low-frequency user action. No-op when the
   * session is not materialized or has no events past `atSeq`.
   */
  async deleteFrom(id: SessionId, atSeq: number): Promise<void> {
    const path = await this.findLog(id)
    if (path === undefined) return
    await this.ensureRootEncoding()
    const buffer = await readFile(path)
    const scan = this.compression === 'zstd' ? undefined : scanLog(buffer)
    if (scan === undefined || scan.events.length === 0) {
      // zstd or unparsable: fall back to loadStored semantics (read then rewrite)
      const stored = await this.loadStored(id)
      if (stored === undefined) return
      const kept = stored.events.filter(e => e.seq <= atSeq)
      const content = await this.encodeMaterialization(stored.meta, kept)
      await this.overwriteLog(path, content)
      return
    }
    const kept = scan.events.filter(e => e.seq <= atSeq)
    if (kept.length === scan.events.length) return // nothing past atSeq
    const content = await this.encodeMaterialization(scan.meta, kept)
    await this.overwriteLog(path, content)
  }

  /** Rewrite an existing log file durably (temp + fsync + rename). */
  private async overwriteLog(path: string, content: Buffer | string): Promise<void> {
