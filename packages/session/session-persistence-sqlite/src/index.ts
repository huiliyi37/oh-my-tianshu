 ...
 * @module @huiliyi37/dsh-session-persistence-sqlite
 */
...
export class SessionPersistenceSqlite extends SessionPersistence implements PersistenceBackend<number> {
  ...
  /**
   * Durably delete stored events with `seq > atSeq` (rewind): one transaction,
   * then bump the stored revision so load-side change detection sees it.
   */
