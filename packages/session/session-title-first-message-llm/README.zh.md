...
该插件使用完整且必填的[共享大语言模型（LLM）配置](../session-title-llm/README.md#configuration)。路由解析遵循共享辅助模块的顺序：可选 `ctx.modelRoles` 服务中的 `secondary` 角色 pin 优先，其次是配置的 `provider`/`model` 对，最后才继承当前已记录主请求的确切路由。
