 ...
 * @module @huiliyi37/dsh-session-persistence/coordinator
 */
...
export interface PersistenceBackend<TornMarker = unknown> {
  ...
  /**
   * Durably delete stored events with `seq > atSeq` (rewind): the session log
   * was truncated in memory and this keeps the durable copy consistent, so a
   * restart does not resurrect dropped history. Keeps the header and the
   * surviving prefix; bumps the stored revision so load-side change detection
   * sees the mutation. No-op when the session is not materialized.
   * @param id - persisted session id to truncate.
   * @param atSeq - inclusive last kept seq (-1 clears all events).
   */
  deleteFrom(id: SessionId, atSeq: number): Promise<void>
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  /**
   * Durably truncate a stored session to `atSeq` (rewind): delete events past
   * the cut on the backend AND reset the in-memory cursor to `atSeq + 1`, so
   * the next append resumes where the truncated log ends (appendCore's
   * contiguity check requires the cursor to match). Without the cursor reset,
   * a new event with `seq = atSeq + 1` would fail `seq !== cursor + i`.
   * @param id - persisted session id to truncate.
   * @param atSeq - inclusive last kept seq (-1 clears all events).
   */
  async truncateStored(id: SessionId, atSeq: number): Promise<void> {
    const state = this.states.get(id)
    await this.backend.deleteFrom(id, atSeq)
    if (state !== undefined) {
      state.cursor = atSeq + 1
      state.materialized = atSeq >= 0
    }
    this.preparations.invalidate(id)
  }
