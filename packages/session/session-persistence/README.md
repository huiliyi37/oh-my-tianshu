# @huiliyi37/dsh-session-persistence
...
| `list(signal?): Promise<SessionListEntry[]>` | Lightweight listing from metadata, no full-log parse. Each entry pairs the header with the backend-tracked durable last-append time when it can serve one from listing metadata (never a full-log read). The optional signal cancels backend listing work. A zero-event lazily-materialized session is absent from `list`. |
