 ...
 * @module @huiliyi37/dsh-session-persistence/tests/coordinator-contract
 */

import { describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@huiliyi37/cordis'
import { scopeTarget } from '@huiliyi37/dsh-scope'
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId } from '@huiliyi37/dsh-session'
...
export function runCoordinatorContract(name: string, makeFixture: () => Promise<CoordinatorFixture>): void {
  describe(`PersistenceCoordinator orchestration: ${name}`, () => {
    ...
    it('truncateStored 截断持久化事件并重置 cursor（后续 append 续 seq 且 reload 后不复活）', async () => {
      const fix = await makeFixture()
      const { ctx, fiber } = await freshCtx(fix)
      try {
        const id = SessionId('rewind')
        const session = ctx.sessions.create(id, { meta: { cwd: WORK } })
        send(session, oneTurnLog()) // 6 events, seq 0..5
        await ctx.sessions.flush(session)

        // 内存 + 持久化双侧截断到 seq 2（保留前 3 个事件）
        session.truncate(2)
        await ctx.sessionPersistence.truncateStored(id, 2)

        // 新 append 续 seq：seq 3 起（cursor 已重置）
        session.append('turn/end', { turn: 1, reason: { kind: 'blocked' } })
        await ctx.sessions.flush(session)

        // 跨 reload 验证：live 会话释放后，磁盘只含截断后的 4 个事件
        await fiber.dispose()
        const { ctx: ctx2, fiber: fiber2 } = await freshCtx(fix)
        try {
          const reloaded = await ctx2.sessionPersistence.load(id)
    ...
    it('truncateStored 对未持久化会话为 no-op（不抛错）', async () => {
      const fix = await makeFixture()
      const { ctx, fiber } = await freshCtx(fix)
      try {
        await expect(ctx.sessionPersistence.truncateStored(SessionId('ghost'), 5)).resolves.toBeUndefined()
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
