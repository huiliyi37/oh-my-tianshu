...
/**
 * Submit the invocation's composer images as one model-visible user message
 * ahead of the goal's next round. The images precede a fixed text block
 * naming their role, so a later goal round reads them from ordinary session
 * history without the goal domain storing attachment state.
 */
