 ...
 * two-section mapping (references and records) rather than a dotenv file: a
 * store the Harness owns and never materializes into the environment cannot
 * also serve as the user's environment layer, and conflating the two is what
 * made a non-secret in the old `$DSH_HOME/.env` silently unreachable.
...
import { canonicalizeWatchPath, resolveDshHome } from '@huiliyi37/dsh-paths'
import { environmentOf } from '@huiliyi37/dsh-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@huiliyi37/dsh-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
...
export class CredentialsLocal extends CredentialProvider {
