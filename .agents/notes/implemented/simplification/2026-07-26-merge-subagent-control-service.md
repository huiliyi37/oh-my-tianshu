...
`SubagentService` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@huiliyi37/dsh-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@huiliyi37/dsh-tool-subagent-control` package injects `ctx.subagents` directly.
