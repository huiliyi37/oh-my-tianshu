...
旧 Agent Note 已经推迟了独立的 `@huiliyi37/dsh-fs-policy` 包。本 Agent Note 构建该层，使 `ctx.fs` 保持接近 fsspec 风格的存储原语（`info`/`cat`/`open`），但不把它变成完整的 fsspec。
...
`@huiliyi37/dsh-fs` 收缩为提供方文本 IO 加受保护的文本变更：
