...
Plan mode 拥有一个 plan 专用产品包：位于 `packages/plan/plan-mode/` 的 `@huiliyi37/dsh-plan-mode`。持久化事实为 `plan/mode: { active: boolean }`，由 `foldPlanMode(events)` 折叠，空日志值为 `false`。`ctx.planMode.get(agent)` 返回 `{ active, pending? }`，`set(agent, active)` 则记录在边界生效的选择。pre-step、重试、追加失败和 dispose（资源释放）栅栏保留相同的状态转换归属。

配置为 `{ section: string }`，另有可选的 `blockedTools` 守卫增补。该包在 plan mode 激活期间把配置的引导作为插件来源通知注入每轮第一个请求的尾部——绝不进入系统提示词——并注册 `/plan [message]`、精确匹配的 `/plan off` 主动退出形式，以及 `exit_plan_mode`。不带参数的 `/plan` 选择激活；其他非空参数则先选择激活，再通过 `agent.steer()` 发送去除首尾空白后的文本，使该文本在受影响的步骤中成为一条记录到日志的普通用户消息。`/plan off` 选择未激活，不产生模型输入，并可取消仍待在边界生效的进入选择。即使 plan mode 未激活，退出工具仍保持注册，以确保请求工具目录稳定。
...
激活状态在每轮第一个请求的尾部贡献部署提供的引导；未激活状态不贡献任何内容。引导绝不进入系统提示词，而 `exit_plan_mode` 在两种状态下都保持注册，因此状态转换既不改变已记录的请求头，也不改变原生工具 schema 或 Code Mode SDK——缓存的请求前缀在转换前后逐字节恒定。用户发起的转换只会在上一条请求头描述相反状态时追加一条来源为插件的通知；第一次请求前的选择或最终状态未变化的选择不会追加通知，经批准的工具退出则依赖其工具结果，不再追加第二条通知。
...
- 通用的 `mode/set` 与 `mode:policy` 名称；plan 包拥有 plan 前缀的词汇（`plan/mode`、`plan/file`）。
...
- 无密钥 Web e2e 场景经真实的 `/plan` 命令进入：control-row 弧线经 chip 的 `/plan off` 走真实命令通道退出；评审弧线提交 `/plan <task>`，记录的轮次止于 `exit_plan_mode`，并经决策卡片批准。
- 完整的 `exit_plan_mode` 评审流程既有包测试，也由无密钥的 `plan-review.e2e.ts` Web 快照端到端覆盖。
...
Plan 状态仍可重建，工具 schema 仍保持稳定，但如果进程在下一边界前退出，空闲状态下待生效的选择会丢失。进入或离开 plan mode 保持缓存请求前缀逐字节恒定——引导随请求尾部注入，绝不进入系统提示词——且在模式激活期间，即使模型忽略引导，单调工具守卫也会拒绝变更工具族。
