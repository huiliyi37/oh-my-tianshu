...
The old Agent Note already deferred a separate `@huiliyi37/dsh-fs-policy` package. This Agent Note builds that layer and keeps `ctx.fs` close to fsspec-style storage primitives (`info`/`cat`/`open`), without turning it into full fsspec.
...
`@huiliyi37/dsh-fs` shrinks to provider text IO plus guarded text mutation:
