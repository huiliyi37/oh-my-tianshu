...
`@huiliyi37/dsh-invariants` 是与产品无关的 Cordis 服务插件，注册 `ctx.invariants`。它只负责配置、注册唯一性、子 fiber 生命周期和带包归属的失败；不导入 session、agent、scope 或 agent-loop 包，也不包含这些包的检查。
...
| 伴随入口 | 注册名 | 所属检查 |
|---|---|---|
| `@huiliyi37/dsh-session/invariant` | `@huiliyi37/dsh-session` | 会话序号、turn/step 包围关系和同 step 的 call/result 轨迹 |
| `@huiliyi37/dsh-agent/invariant` | `@huiliyi37/dsh-agent` | agent 状态转换 |
| `@huiliyi37/dsh-scope/invariant` | `@huiliyi37/dsh-scope` | scoped event carrier 存在性与主体一致性 |
| `@huiliyi37/dsh-agent-loop/invariant` | `@huiliyi37/dsh-agent-loop` | 模型请求重建 |
