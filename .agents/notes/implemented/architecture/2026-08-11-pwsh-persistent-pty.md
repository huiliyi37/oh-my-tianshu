...
The harness had no persistent shell on Windows. The persistent `bash` stack was POSIX-only by construction: `@huiliyi37/dsh-subprocess-local` threw at terminal allocation (`createProcessInspector()` rejected win32), `@huiliyi37/dsh-pty-local` was bash-shaped (`/bin/bash` defaults, `PS1`/`PROMPT_COMMAND` environment markers), `@huiliyi37/dsh-tool-bash-persistent` wrapped commands in bash syntax, and every pty test skipped on win32. The one-shot `pwsh` tool (`@huiliyi37/dsh-tool-pwsh` over `@huiliyi37/dsh-pwsh-local`) already ran on Windows, but each call started a fresh `pwsh -Command` process: cwd, `$env:` variables, functions, and interactive children ended with the call, and its README recorded "No persistent shell or PTY" as deferred work.

The gap excluded Windows workflows whose state lives in a terminal: stepping a debugger, exploring in a Python or Node REPL, or returning to a shell after interrupting its foreground command — the same class of work the [persistent PTY sessions](../feature/2026-07-16-persistent-pty-sessions.md) decision serves on POSIX.

Two foundations already existed. The PTY service itself (`ctx.pty` registry, owner scoping, send/read/signal/kill contract) is platform-neutral. The Loader's `disabled: !!js` interpolation gates shell rows per platform and pins the invariant that exactly one persistent shell stack mounts per host; a persistent pwsh stack composes through the same rows.
...
### Windows substrate in `@huiliyi37/dsh-subprocess-local`
...
### Shell dialect in `@huiliyi37/dsh-pty-local`
...
A new package under `packages/pty/` mirroring `tool-bash-persistent`: same `Config` (`backendType` default `shell`, `timeoutMs`, `maxOutputChars`, `description`), same owner-scoped shell registry and serialized per-owner queue, same timeout/abort/exit/reset paths. The tool name is `pwsh`; in the shipped presets it never co-mounts with the one-shot `tool-pwsh` because the minimal preset's persistent rows are mutually exclusive per platform.
...
The minimal preset gates its persistent shell stack by platform with the Loader's `disabled: !!js` interpolation: the bash rows (`terminal-bash` + `tool-bash-persistent`) mount on POSIX, and the pwsh rows (`terminal-pwsh` with `shellDialect: pwsh` + `tool-pwsh-persistent`) mount on win32 — exactly one persistent shell per host. `windows-shell.spec` pins the per-platform roster; the package's loader-composition suite exercises the whole stack over a real ConPTY pwsh where one is installed. The one-shot shell tools are a separate local decision: standard/code/cordis mount `tool-bash`/`tool-pwsh` unconditionally and leave platform absence to the executors.

### Testing

The Windows test surface keeps the exemption structure: pty-local and subprocess-local tests stay excluded on win32 (`windowsUnsupportedTests`) and their sources stay coverage-exempt there (`windowsUnsupportedCoveragePackages`), so the platform-gated fixtures and node-translated commands remain the win32 dev-lane evidence, while the koffi-backed inspector joins the windows-only coverage exclusions on Linux and macOS. `tool-pwsh-persistent` is not exempt: its suite runs and its sources are coverage-required on the windows-native lane, mirroring `tool-bash-persistent`'s stub-mode matrix plus an echo-stripping mode; the real-pwsh suites prove persistent cwd/env, secret scrubbing, multiline and here-string commands, large-output clipping, and exit/reset over real ConPTY sessions and self-skip on hosts without pwsh. The tool-catalog generator boots the tool through its mount and pins its model-visible schema in `docs/tool-catalog.md`; a keyless ACP snapshot of the persistent pwsh tool (upstream `cef99b17d4`) is not part of this port.
...
**Windows became a first-class persistent-shell host.** The persistent pwsh stack runs and is coverage-gated on the windows-native lane; the minimal preset's spec pins exactly one persistent shell stack per host on both platforms.

**Windows coverage keeps the exemption structure.** subprocess-local and pty-local sources stay coverage-exempt and their suites test-excluded on win32 exactly as before; the Windows code paths are exercised through the win32 dev lane and the real-pwsh tool suites, and the new surface's coverage obligation on the windows-native lane sits on `tool-pwsh-persistent`.
