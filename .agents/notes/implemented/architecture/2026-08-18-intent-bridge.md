...
- **Handoff is create-then-commit.** `AlignState.finalizing` is a synchronous in-flight sentinel that serializes the tool, forced, and fallback paths across the create window; `finalized` is set only after the card reached the main session. A failed create or delivery surfaces as a tool error and stays retryable; a main session that was created but never received the card is disposed so the retry mints a fresh one (the invariant forbids a second handoff record on the same session).
...
- 31 package tests + snapshot green on macOS; zen (61), task-card (28), and TUI app (260) suites re-run without regression.
...
- `8b93a9640c` — create-then-commit handoff: `finalizing` in-flight sentinel, retryable failed create/delivery, card-less main session disposed (3 new tests; README Known Limitation rewritten).
