...
| 包 | 角色 |
|---|---|
| `@huiliyi37/dsh-spill` | 接口：`ctx.spillStore`、词汇类型，不包含存储实现。 |
| `@huiliyi37/dsh-spill-local` | 本地后端：在宿主文件系统中提供私有、会话作用域的文件存储。 |
| `@huiliyi37/dsh-spill-policy` | 工具结果策略插件：包装分发后的最终文本结果，并以保留预览和落盘定位符替换超大结果。 |
...
- `@huiliyi37/dsh-retention` 负责预览机制（`TextRetainer`、`ItemRetainer` 和省略元数据）。
- `@huiliyi37/dsh-spill` 负责保存最终文本，并返回定位符与检索提示。
- `@huiliyi37/dsh-spill-policy` 在工具流水线中应用默认的最终结果策略，将前两者组合起来。
