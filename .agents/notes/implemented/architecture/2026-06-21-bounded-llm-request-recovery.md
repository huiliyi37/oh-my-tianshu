...
`@huiliyi37/dsh-llm` exports one JSON-serializable `LlmFailure` payload:
...
`@huiliyi37/dsh-llm-retry` is a function plugin that listens to `agent/request-error`. It introduces no service or new loop branch; the agent-loop package changes only the data carried through its existing failed-step recovery control flow.
