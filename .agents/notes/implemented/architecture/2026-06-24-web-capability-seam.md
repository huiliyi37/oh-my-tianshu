...
1. `@huiliyi37/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@huiliyi37/dsh-web-search-exa`, `@huiliyi37/dsh-web-search-perplexity`, `@huiliyi37/dsh-web-search-deepseek`, and `@huiliyi37/dsh-web-fetch-local`.
3. `@huiliyi37/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@huiliyi37/dsh-tool-web  --depends on-->  @huiliyi37/dsh-web  <--depends on--  @huiliyi37/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @huiliyi37/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @huiliyi37/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @huiliyi37/dsh-web-fetch-local
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@huiliyi37/dsh-web-search-exa"] -->|registerSearchProvider| web["@huiliyi37/dsh-web / ctx.web"]
  perplexity["@huiliyi37/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@huiliyi37/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@huiliyi37/dsh-web-fetch-local"] -->|registerFetchProvider| web
  toolWeb["@huiliyi37/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@huiliyi37/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@huiliyi37/dsh-tool-web` depends on `@huiliyi37/dsh-web`, `@huiliyi37/dsh-tools`, `@huiliyi37/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@huiliyi37/dsh-web'
...
- id: web-search-deepseek
  name: '@huiliyi37/dsh-web-search-deepseek'

- id: web-fetch-local
  name: '@huiliyi37/dsh-web-fetch-local'
