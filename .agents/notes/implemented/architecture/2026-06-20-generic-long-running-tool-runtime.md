...
- `@huiliyi37/dsh-tasks` registers running work as `ctx.tasks` and owns task ids, authorization, snapshots, reads, cancellation, waiting, completion listeners, and cleanup.
- `@huiliyi37/dsh-tool-tasks` exposes `task_output`, `task_list`, and `task_kill`, injects completion notices, and supplies the background-task system-prompt guidance.
...
`TaskService` is the Service Definition in `@huiliyi37/dsh-tasks`; the process-local provider is `LocalTaskService` in `@huiliyi37/dsh-tasks-local` (the [task-registry contract Agent Note](2026-07-26-task-registry-seam.md) records that split).
