...
The event vocabulary lives in contract packages (`dsh-agent` declares the `agent/*` events); `@huiliyi37/dsh-agent-loop` is the only concrete loop plugin and is itself swappable — nothing outside it may depend on it.
