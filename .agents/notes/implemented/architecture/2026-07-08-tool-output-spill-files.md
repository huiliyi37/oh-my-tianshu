...
| Package | Role |
|---|---|
| `@huiliyi37/dsh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@huiliyi37/dsh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@huiliyi37/dsh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-policy
  name: '@huiliyi37/dsh-spill-policy'
  config:
...
- `@huiliyi37/dsh-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@huiliyi37/dsh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@huiliyi37/dsh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
