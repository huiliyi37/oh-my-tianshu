...
harness 在 Windows 上没有持久 shell。持久 `bash` 栈按构造就是 POSIX-only：`@huiliyi37/dsh-subprocess-local` 在终端分配时直接抛错（`createProcessInspector()` 拒绝 win32），`@huiliyi37/dsh-pty-local` 是 bash 形态（`/bin/bash` 默认值、`PS1`/`PROMPT_COMMAND` 环境标记），`@huiliyi37/dsh-tool-bash-persistent` 用 bash 语法包装命令，pty 测试全部在 win32 上 skip。一次性 `pwsh` 工具（`@huiliyi37/dsh-tool-pwsh` + `@huiliyi37/dsh-pwsh-local`）已经能在 Windows 运行，但每次调用都是全新的 `pwsh -Command` 进程：cwd、`$env:` 变量、函数和交互式子进程都随调用结束，其 README 把 "No persistent shell or PTY" 记为 deferred work。

这个缺口排除了状态驻留在终端里的 Windows 工作流：单步调试、在 Python 或 Node REPL 中探索、中断前台命令后回到原 shell —— 正是[持久 PTY 会话](../feature/2026-07-16-persistent-pty-sessions.md)决策在 POSIX 上服务的同一类工作。

两个基础已经存在。PTY 服务本身（`ctx.pty` 注册表、owner 作用域、send/read/signal/kill 契约）是平台无关的。Loader 的 `disabled: !!js` 插值按平台门控 shell 行，并钉死了「每宿主恰好挂载一个持久 shell 栈」的不变量；持久 pwsh 栈通过同一行机制组合。
...
### `@huiliyi37/dsh-subprocess-local` 的 Windows 基座
...
### `@huiliyi37/dsh-pty-local` 的 shell 方言
...
新包位于 `packages/pty/`、镜像 `tool-bash-persistent`：同样的 `Config`（`backendType` 默认 `shell`、`timeoutMs`、`maxOutputChars`、`description`）、同样的 owner 作用域 shell 注册表与每 owner 串行队列、同样的超时/中止/退出/重置路径。工具名是 `pwsh`；在发货预设中它与一次性 `tool-pwsh` 永不共挂，因为 minimal 预设的持久 shell 行按平台互斥。
...
minimal 预设用 Loader 的 `disabled: !!js` 插值按平台门控持久 shell 栈：bash 行（`terminal-bash` + `tool-bash-persistent`）在 POSIX 挂载，pwsh 行（`shellDialect: pwsh` 的 `terminal-pwsh` + `tool-pwsh-persistent`）在 win32 挂载——每宿主恰好一个持久 shell。`windows-shell.spec` 钉死按平台的花名册；包自带的 loader-composition 套件在装有真实 ConPTY pwsh 的主机上跑通整条栈。一次性 shell 工具是另一项本地决策：standard/code/cordis 无条件挂载 `tool-bash`/`tool-pwsh`，平台缺位交给执行器处理。

### 测试

Windows 测试面沿用豁免结构：pty-local 与 subprocess-local 的测试在 win32 上继续排除（`windowsUnsupportedTests`），其源码在 win32 上继续覆盖豁免（`windowsUnsupportedCoveragePackages`），平台门控 fixture 与 node 翻译命令因此仍是 win32 开发车道的证据；koffi-backed inspector 在 Linux 与 macOS 侧加入 windows-only 覆盖豁免。`tool-pwsh-persistent` 不在豁免之列：其套件在 windows-native 车道上运行、源码受覆盖约束，镜像 `tool-bash-persistent` 的 stub 模式矩阵并加回显剥离模式；真实 pwsh 套件在真实 ConPTY 会话上证明持久 cwd/env、密钥清洗、多行与 here-string 命令、大输出裁剪与退出/重置，并在没有 pwsh 的主机上自跳过。tool-catalog 生成器通过工具的 mount 启动它，并在 `docs/tool-catalog.md` 固定其模型可见 schema；持久 pwsh 工具的 keyless ACP 快照（上游 `cef99b17d4`）不在本次移植范围内。
...
**Windows 成为一等公民的持久 shell 宿主。** 持久 pwsh 栈在 windows-native 车道上运行并受覆盖门禁约束；minimal 预设的 spec 在两种平台上都钉死每宿主恰好一个持久 shell 栈。

**Windows 覆盖沿用豁免结构。** subprocess-local 与 pty-local 源码在 win32 上保持覆盖豁免、其套件保持测试排除，与此前的结构完全一致；Windows 代码路径经 win32 开发车道与真实 pwsh 工具套件验证，新表面的覆盖义务在 windows-native 车道上落在 `tool-pwsh-persistent`。
