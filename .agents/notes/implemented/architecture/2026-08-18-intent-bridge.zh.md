...
意图对齐桥（`packages/guard/intent-bridge`，`@huiliyi37/dsh-intent-bridge`，ctx key `intentBridge`）拆分职责：可配置的**对齐模型**在专用对齐会话中与用户做多轮意图对齐；意图清晰后调用 `finalize_alignment`，桥把结构化任务卡（task-card 契约）交给**全新主会话**。主会话从不继承对齐上下文——只接收任务卡——因此它是干净的顶层会话：task-card 幂等（不改写）、zen triage 不跳过多行任务卡、主模型在禅相位锚定后解锁全量工具面。
...
- **主模型纯提示引导**——否决：模型侧纪律，无法强制执行；仅靠引导救不了过宽的工具面。
