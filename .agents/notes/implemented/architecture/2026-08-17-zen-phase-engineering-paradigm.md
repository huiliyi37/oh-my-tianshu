...
- Zen-phase hiding reaches only the tools a deployment still mounts. The TUI unmounts its memory and session-query tool rows outright, so those are absent from every request rather than hidden for a while; for what stays mounted, the triage heuristic and step budget bound how long the phase hides it.
- The TUI top status bar renders a `禅` badge while the phase is armed (`zenPhaseLabel` over the logged `zen/phase` fold in `preset-surface.ts`; it disappears on promotion or compaction prune). The package defaults remain the official minimal recipe + `todo_write` and a 4-step budget; the TUI patch overrides the face with the minimal probe pair `[bash, read]` and a curated `promoteDeny`. Retrieval stays deferred because the axis is overlap, not count.
...
- `packages/tui/tui/tests/bundle-patch.spec.ts` — the `zen` row's non-empty section, its `[bash, read]` face, its `promoteDeny` derived from `BASH_OVERLAP_TOOLS`, the two lists' disjointness, and diet; plus the three in-place tool-row disables and the six id-keyed base overrides, each checked against the base patch for a live id/name pair.

## Related

- [Prompt sections follow the tool face](../bug-fix/2026-08-22-prompt-sections-follow-the-tool-face.md) — the prompt-coherence invariant that prunes each `tool:<name>` section with the face, and the shipped TUI face composition.
