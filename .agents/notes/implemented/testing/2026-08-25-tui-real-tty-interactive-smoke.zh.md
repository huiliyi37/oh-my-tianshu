...
**对 PTY 原始字节匹配标记。** 否决：ANSI 组帧可能伪造通过；标记匹配在解析后的活动缓冲（正常或备用屏）上进行。

**复用 `welcome-driver.ts` 作夹具。** 否决：其启动 Tip 的 `Math.random` 守卫假定欢迎页前无原生插件加载；bash 沙箱栈加载 koffi 会触发它——夹具驱动是朴素的 boot-and-yield。

**在无宿主 PTY 处自我跳过。** 否决：本泳道在标准无 key 快照泳道中、凡有宿主 PTY 处运行（CI runner、开发机）；仅 DSH agent 沙箱（posix_openpt 被拒）需要宿主 TTY 调用。
