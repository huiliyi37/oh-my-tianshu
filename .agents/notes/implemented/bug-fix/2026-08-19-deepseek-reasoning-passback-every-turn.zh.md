...
随移植一并保留的本地分叉：在 `deepseek-spark` 路由上，spark 尾部截断（`spark.truncateN`）随该字段的发出位置一并生效，因此放宽后的回传在该路由上仍然有界；spark 之外的路由逐字回传不截断。
...
`tests/serialize.spec.ts` 固定了三种 assistant 形态：推理与文本并存且无工具调用、推理与工具调用并存、以及内容保持为 `""` 的纯推理轮次。不携带推理的轮次仍不发出该字段，由无内容与仅工具调用两种用例覆盖。`tests/spark-wire.spec.ts` 的纯文本轮用例同步反转到新规则：字段存在，且在 spark 路由上仍按配置截断尾部。
