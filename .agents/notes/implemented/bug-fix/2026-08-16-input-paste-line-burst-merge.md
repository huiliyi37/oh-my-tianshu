...
`InputHandler` now tags a `return` key with `inline: true` when the same input buffer still holds bytes after it (`dispatchKeys`: `i + consumed < buf.length`). A user's Enter arrives alone at the end of a chunk; a pasted line's CR is followed by the next line's text in the same flush. A return that lands at the end of a chunk **after other bytes in that same chunk** is held for 12ms: if another `data` event arrives in that window, the held return is flushed as inline, so a terminal that splits a paste into one line per flush no longer submits each line. A lone CR (the usual user Enter) dispatches immediately; Shift/Meta/Ctrl+Enter are never held. `InputLine` treats an inline return as a line separator: it accumulates the current value (`_inlinePasteLines`) and clears the input line without submitting; the next ordinary return (buffer exhausted after the hold — i.e. the paste's final CR or the user's own Enter) merges all accumulated lines with `\n` into a single submit. The Vim-normal return path merges identically. Bracketed-paste flow is untouched (it never produces inline returns); a normal single Enter is unchanged (a lone CR dispatches immediately); rapid manual double-Enter stays two separate submits.
...
Known residual: a paste whose flush gap exceeds 12ms still submits per flush; bracketed-paste-capable terminals never hit this path.
