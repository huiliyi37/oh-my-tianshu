...
[English](2026-08-18-safari-textarea-soft-wrap-reflow.md) | 中文

移植自上游 deepseek-ai/deepseek-harness `4edbdd443`（dsh-v0.1.0-rc.7 缺陷修复回移）；组件测试与浏览器识别测试一并移植。
...
上游修复还在 Safari 26.5.2 中通过原生的 51 字符到 50 字符 Backspace 路径验证过。Playwright WebKit 26.5 在组装应用与最小化页面中都无需本绕法即可正确稳定，因此仓库的 Chromium 浏览器泳道无法复现这个 Safari 应用缺陷；在可自动化的 Safari 泳道出现之前，由聚焦组件测试固定该引擎状态。
