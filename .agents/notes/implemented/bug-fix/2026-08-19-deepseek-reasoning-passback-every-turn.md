...
Local divergence carried along: on the `deepseek-spark` route the spark tail truncation (`spark.truncateN`) applies wherever the field is emitted, so the widened passback stays bounded there; off spark routes the text passes through untruncated.
...
`tests/serialize.spec.ts` pins all three assistant shapes: reasoning beside text with no tool call, reasoning beside a tool call, and a reasoning-only turn whose content stays `""`. Turns carrying no reasoning keep emitting no field, which the content-less and tool-call-only cases cover. `tests/spark-wire.spec.ts` flips its plain-turn case to the new rule: the field is present and still truncated to the configured tail on the spark route.
