# Agent Note: TUI chrome closed loops — Phase 1 pins

Status: implemented

English | [中文](2026-08-26-tui-chrome-closed-loops.zh.md)

Scope: `packages/tui/tui` (`ApprovalController` consumers, `LiveSnapshot`, `render/live-panels`, `TuiApp.attach` / `renderLive`)
...
TUI tests now pin persist-then-abort, restart-still-asks, `allowed-always` mode-cycle / `/yolo`, snapshot-driven activity rows, and no handoff listener when the bridge is off. Row-budget idle-skip (Phase 2) is not in this change. Maintainers must not fold activity beside the snapshot again, and must not subscribe `intent-bridge/handoff` on a disabled plugin.
