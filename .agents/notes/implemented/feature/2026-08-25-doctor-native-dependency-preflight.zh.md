...
**经 vitest 路径映射解析探测。** 否决：它不能替代 plain Node 的解析；TUI 包声明 subprocess-local 属主，使 `node --import tsx/esm` 与用户运行方式解析一致。

**只靠 README 补救（不加 /doctor 行）。** 否决：README 记录了 `--allow-scripts` 重装，但没有任何面在诊断点把它呈现出来。

本检查报告可加载性而非构建健康（能加载但损坏的二进制不在范围内），TUI 组合本身也无需这些模块即可启动——本预检只说明环境是否就绪。
