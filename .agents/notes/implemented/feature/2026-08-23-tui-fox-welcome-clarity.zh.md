...
- `80 ≤ cols < 89`：56 档，平级行与后续详情折行
- `89 ≤ cols < 105`：56 档，单行平级文案
- `cols ≥ 105` 且行数够承载 `72×27` 单元格加 chrome：72 档

身份栏以一行主题色 `Oh My Tianshu` 开头。没有五行块字 wordmark。平级文案仍是 `DeepSeek ◆ Tianshu Harness`。
...
买到的：80 列上狐狸可辨认，105 列更清晰；折行保住身份可读，而不是挤压画面；挂载不再把 3.24 秒花在开场上。
...
- formatter 覆盖钉住欢迎路径只用半块、拒绝其他宽度，以及 80 折行 / 89 中档 / 105 宽档 / 文本回退。
