...
运行时欢迎画面是两张 contain 休息网格，吸附到同一套至多 16 色的平面调色板，且不做误差扩散。档高、坐姿抠图、nearest-neighbor 核与 splash 文案由[坐姿狐狸欢迎](./2026-08-25-tui-sitting-fox-welcome.md)拥有。`formatFoxFrame` 只用半块绘制这些网格，并拒绝其他宽度。`formatWelcomeHero` 按列数与行数选档：

- 低于 80 列，或行数放不下窄档单元格加 chrome，或无彩色 / 窄块字形：紧凑文本
- `80 ≤ cols < 105`：窄档，标题留在狐狸右侧
- `cols ≥ 105` 且行数够承载宽档单元格加 chrome：宽档

身份栏留在狐狸右侧。没有五行块字 wordmark，标题也不堆到画面上方。
...
- controller 与 app 覆盖钉住立即 complete、`auto` 等于 `off`、105×25 的宽档，以及恢复先于欢迎 / 挂起动作顺序。
- 真实 [`examples/tui`](../../../../examples/tui/README.md#keyless-snapshot) Loader + PTY 快照记录结算后的 100×40 中档面，含半块且无盲文。
