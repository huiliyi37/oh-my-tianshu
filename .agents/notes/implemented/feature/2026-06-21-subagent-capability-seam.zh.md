...
| 包 | 角色 |
|---|---|
| `@huiliyi37/dsh-subagent` | 接口：`SubagentService`（`ctx.subagents`）、`SubagentProvider`、`SubagentRun`、请求、结果、能力词汇、`subagent/*` 事件 |
| `@huiliyi37/dsh-subagent-spawn` | 实现：通过 `ctx.agents.create` 创建全新的进程内子 agent |
| `@huiliyi37/dsh-subagent-fork` | 实现：用父 agent 日志快照初始化的进程内子 agent |
| `@huiliyi37/dsh-subagent-acp` | 实现：作为 ACP 客户端驱动已配置的子进程 |
| `@huiliyi37/dsh-subagent-codex` | 实现：一次性官方 Codex app-server 进程 |
| `@huiliyi37/dsh-subagent-claude-code` | 实现：通过 Agent SDK 运行的一次性官方 Claude Code 进程 |
| `@huiliyi37/dsh-tool-subagent` | 消费方：基于 `ctx.subagents` 的面向模型的 `subagent` 工具 |
...
全新子 agent 与 fork 子 agent 是独立的提供方，而非请求中的一个 flag。`dsh-subagent-spawn` 启动隔离的子 agent；`dsh-subagent-fork` 用一个平衡前缀初始化子 agent，该前缀仅包含已完成的父轮次。进行中的轮次被排除，因为其 subagent 调用尚无结果，无法构成有效的回放历史。
