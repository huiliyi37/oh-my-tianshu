...
| Package | Role |
|---|---|
| `@huiliyi37/dsh-subagent` | interface: `SubagentService` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@huiliyi37/dsh-subagent-spawn` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@huiliyi37/dsh-subagent-fork` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@huiliyi37/dsh-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@huiliyi37/dsh-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@huiliyi37/dsh-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@huiliyi37/dsh-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
