...
- **Budget compute-and-record** (closed-loop Phase 3, option a): `budget` config (validated, no hardcoded tunables) feeds the Tianshu-shaped pricing (`shapeWriteBudget` turns by file count + double ceilings; wall-clock is the single-shot cap); the route record and `DispatchOutcome` carry `{ maxTurns, deadlineMs }`. Enforcement is a future seam capability (see the algorithm-candidates proposed note).
- **Adaptivity gate pure function** (closed-loop Phase 4): `promotion.ts` ships `effectivePromotionMode` (kill-switch-first four-mode semantics) and `resolvePromotionGate` (samples → false-green → scope-health → margin veto ladder, `MIN_SAMPLES=30`/`MIN_MARGIN=0.05`); shadow tally wiring and auto mode are registered as candidates, not shipped.
...
- Package-level tests: 95 all green (prediction 18 / router 12 / dispatch 8 / integration 19 / invariant 17 / synthesis 9 / budget 6 / promotion 6).
