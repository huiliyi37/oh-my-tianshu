...
- **Primary-agent synthesis** (closed-loop Phase 2): a `router:synthesis` prompt section renders when unresolved child findings exist (derived purely from the logged `router/outcome` minus `router/adoption` records — model-visible ⟺ logged); the `router_adopt` tool records adopt/reject declarations as log-only `router/adoption` (at most one per outcome, enforced at the tool boundary and by the invariant companion's per-session pairing state). The router never merges or votes — synthesis is the main agent's act. A verification gap (file mutation without a fresh `run_tests`/`related_tests`) appends a soft reminder in the section (claim-audit freshness, advisory only). The plugin declares `inject: ['tools', 'systemPrompt']` and adds `dsh-tools`/`dsh-system-prompt` peer dependencies plus tsconfig references. Both contributions are gated on dispatchability (`dispatchEnabled` plus explicit `provider`/`model`): on assemblies that cannot dispatch (the shipped TUI's shadow mount) no outcome can exist, the section would render empty and every adopt call would throw — so neither is registered, keeping the model face free of dead weight.
- **Budget compute-and-record plus enforced run bounds**: `budget` config feeds the route's `{ maxTurns, deadlineMs }` pricing record, while auto dispatch separately transfers validated `runBudget { maxSteps, timeoutMs }` through the subagent capability seam. The in-process provider enforces both bounds; the [single-expert rollout note](2026-08-21-tui-single-expert-auto-rollout.md) owns the current terminal and validation contract.
- **Promotion evidence, not autonomous adaptation**: `promotion.ts` exposes kill-switch-first mode resolution plus shadow-readiness and canary-health gates over the durable ledger. Gates only record verdicts; a human still changes configuration. The [single-expert rollout note](2026-08-21-tui-single-expert-auto-rollout.md) owns current sample requirements and veto order.
...
- **Turn-end trigger** (production trigger point): `trigger: { mode, onTurnEnd }` defaults to off. Shadow mode decides at `turn/end` and appends a log-only `router/decision`; auto dispatches through the seam and records `dispatched:true` + `subagentSessionId` at acceptance, before result settlement. Failures before acceptance record `dispatched:false`; infrastructure rejection after acceptance records a paired error outcome without rewriting the accepted decision. Child sessions are excluded. The shipped TUI remains shadow; switching to auto is a product call after the closed loop is verified.

## Key verification facts

- Focused package, integration, and snapshot checks cover the routing table, dispatch lifecycle, durable ledger, synthesis, budgets, promotion gates, and real assembly.
- Snapshot obligation: `examples/headless-agent/tests/headless.snapshot.ts`'s `agent-router-synthesis` scenario pins the assembled transcript — fail-loop escalation → real verifier dispatch → the followup request's `header.system` carries the `router:synthesis` section verbatim → the mock calls `router_adopt` → `router/adoption` lands (route/outcome/adopt-call/adoption chain; budget pinned with the wall-clock deadline normalized to zero).
...
- **Shipped TUI re-mount**: the TUI bundle used to mount agent-router with no config — inert (no production caller of `decide`/`execute`, `execute` short-circuits on missing provider/model) while its `session/event` listener ran unconditionally — so the mount was removed for a while. Phase 1 re-mounts it with `trigger: { mode: 'shadow', onTurnEnd: true }` pinned by the bundle-patch test: turn-end decisions are recorded as log-only `router/decision` without dispatching, and switching to auto is a product call after the closed loop is verified. The headless e2e assembly remains the reference integration.
...
The routing layer is a pure decision function over basic metrics, pinned by package tests and an integration suite on a real Context; prediction still works standalone when evidence-gate is absent, and accounting costs zero new event channels. The price is that everything adaptive stays out: without bandit promotion and the EFE suite, routing reacts only to the configured thresholds and cooldown state the table was written against, and widening it means adding rules, not learning them.

Dispatch inherits the same bounded shape. The profile tool restriction installs fail-loud (the restrict signature is mapped: unknown tool names throw), so a delegated subagent gets only its profile's read-only/verification tool set; the headless e2e covers the verifier subagent's full round trip (`DSH_ROUTER_EXECUTE`); the table carries the profiles it was written with, without doc_scout/verifier variants; and the link to evidence-gate is one-way metric consumption — an escalate creates no obligation, while the delegate attempt itself is recorded (`router/route` at acceptance, `router/decision` at turn-end). Real assembly bought two corrections in exchange: success/failure judgment now reads the `[exit code: …]` text because dsh bash does not mark isError, and evidence-gate exposes `cooldownTable()`/`verificationCount()` for state its tracker already held.
