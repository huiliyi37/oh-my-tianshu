...
验证（全部通过）：identity 9 + telemetry 25 + attachment 16 + schedule 131 + tui commands 135 测试；`tsc -b`（单包 + host 聚合）；`build:lib:host`（tsdown 全量打包，新包 lib 产出，schedule 共享 chunk 随 files 声明发布）；verify-package-paths、verify-package-invariants（212 伴生）、verify-built-package-invariants（212 编译伴生 plain-Node Loader）、verify-package-readme-limitations、verify-package-readme-model-experience（212 README）、verify-translation-pairing、verify-module-graph、verify-export-jsdoc、verify-doc-budgets、verify-lint-budgets（oxlint 全仓 0 诊断）、verify-md-links、verify-doc-refs。

存量失败（与本轮无关、main 同现）：verify-md-wrap 的 docs/2026-08-16-phase2-isolation.md、verify-source-budgets 的 tui app.ts / input-line.ts。

## Deferred

- preset（persona + agent-presets）、pwsh-sandbox、extensions 三件套：后续轮次按序移植。
