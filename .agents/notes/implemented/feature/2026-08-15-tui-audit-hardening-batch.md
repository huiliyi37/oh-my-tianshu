...
- **Vision-bridge probe**: `TuiApp.resolveVisionBridge` treats the presence of the `visionBridge` service as bridge-available when no `vision` config was injected; `packages/context/vision-bridge` provides that service at `apply` (released on unload; `enabled: false` provides nothing). Assemblies may still inject `vision.bridgeEnabled` explicitly — the probe is the no-config fallback.
