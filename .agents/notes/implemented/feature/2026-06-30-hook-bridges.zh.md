...
- **工具输入重写。** CC/Codex 的 `updatedInput` 对 loop 派发的调用经 pre-commit 相位生效（见 [pre-tool-input-rewrite Agent Note](2026-06-30-pre-tool-input-rewrite.md)）：历史、`tool/call` 审计（原始入参入 sidecar）、展示与执行四者同带生效入参；绕过 loop 相位的调用保留降级警告。
