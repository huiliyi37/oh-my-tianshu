...
- **The Cordis 4 injection proxy now has four instances** (the T4 task pane `17e5129`, /compact, evidence-gate tools, and the /preset command — ported in `84e488a3` reading the optional `agentPresets` service by property access, which crashed the real runtime with "without inject" and was reworked to `reflect.get` with a proxy-emulating regression test): optional-service property access throws "without inject" → `ctx.reflect.get(name, false)`; **service-readiness timing problems are declared with `ctx.inject([...])`** (same as tui-runner).
