# Agent Note: dsh spark 推理尾部截断 + 锚点补偿移植（内部能力）
...
[English](2026-08-12-dsh-spark-port.md) | 中文
...
- **与 wire 截断精确互补（同源门控）**：同一 N 同一 tokenizer；N 与总开关 enabled 都从 `llm-deepseek` settings 命名空间读取（`ctx.settings.get`）；无 settings 或 `spark.enabled !== true` 一律不注入（fail-closed）——消除桌面端「两边配置漂移」风险
...
**审查修复（f336a60）**：提交后审查 4 条 CRITICAL 全修——enabled 门控同源 （核心 bug：缺省 enabled=false 时 wire 不截断但锚点仍注入，破坏互补承诺）、 invariants 依赖补全、README 双语配对、settings 同源与门控测试覆盖。

**Wave 3 — 收尾（e280dbe）**：装配冒烟两态（真实 Loader + mock API 断言 wire 截断）+ Agent Note。

**/model 别名（360adc3）**：`spark-flash` / `spark-pro` 一键切换 （→ `deepseek-spark/deepseek-v4-flash` / `deepseek-v4-pro`），别名只做路由展开， 不绕过 enabled 门控；key 与 deepseek 共用（复用主 key，无独立配置）。

## 验证

- Wave 1：spark.spec 25 + spark-wire.spec 14（RED→GREEN）；llm-deepseek 189/189 绿
- Wave 2：anchors.spec 12 + prestep.spec 6（真实装配 + settings 同源门控，不 mock 中间层）；两包 205→207/207 绿
- Wave 3：loader-composition +2 装配冒烟；llm/context 组全量回归
- TUI：commands.spec 98/98、TUI 全量 1418/1418 绿
- typecheck（两包 + TUI tsc exit 0）+ oxlint 0 错
