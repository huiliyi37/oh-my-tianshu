...
**放宽 `a` 的会话内开关而不落规则。** 否决：该开关在审计词汇里不可见——「用户本次允许」与「常设授权放行」的区分正是结果联合类型的意义。

**让单次按键能写通配规则。** 否决：无法解析调用实参的请求 fail loud——无限制的 `*` 不是一次按键应能授予的；工具级规则仍走 `/permissions add` 的显式路径。

**同一变更里放宽 remote/Web decide 联合。** 延后：decide 通道是人类的一次性回答；always-allow 入口另行为 Web 落地（2026-08-27）。
