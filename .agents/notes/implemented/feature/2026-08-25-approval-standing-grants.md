...
**Widen `a`'s session-local flag instead of persisting a rule.** Rejected: the flag was invisible in the audit vocabulary — the split between "the user allowed this once" and "a standing grant allowed this" is the point of the outcome union.

**Let a single keypress author a wildcard rule.** Rejected: requests without resolvable call arguments fail loud — an unrestricted `*` is not something one keypress should grant; `/permissions add` stays the deliberate path for tool-wide rules.

**Widen the remote/Web decide union in the same change.** Deferred: the decide channels are human one-shot answers; the always-allow affordance landed separately as the Web standing grant (2026-08-27).
