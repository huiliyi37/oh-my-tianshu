...
Add the independently installed `@huiliyi37/dsh-tool-subagent-report` package. It contributes an ordinary model-facing `report` tool to each continuable in-process child Activation. A child may call it zero or multiple times in a turn. Success neither concludes the turn, settles the Activation, nor prevents later parent follow-ups, and finishing a turn never reports automatically.
...
The authored snapshot starts a continuable child, executes the real scope-local `report` tool, and observes default next-step delivery wake the parked parent into one turn that consumes the framed report; a snapshot-only fence holds the child behind the parent's delegation turn so the wake is deterministic. A later parent prompt still reads the framed report from the durable log. It declares child schema pin `1`, so the otherwise non-global `report` schema is checked against `tool-schemas.1.expected.json` while the root keeps the default schema pin. The generated tool catalog separately mints a child scope to include the same scope-local schema.
...
Waking on every report creates unsolicited turns and can cascade through nested subagents, so the first version shipped quiet by default. The [ordering decision](../bug-fix/2026-08-17-subagent-report-settlement-ordering.md) reversed that default for next-step delivery: reports waiting together share one step, and a parked parent otherwise never reads an accepted report.
...
- The keyless assembled snapshot proves the real child tool, default next-step wakeup of a parked parent, durable parent framing, and later parent consumption.
