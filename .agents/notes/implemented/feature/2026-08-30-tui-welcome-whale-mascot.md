...
**Reuse an existing big-text/figlet dependency** — no such dependency is vendored, and the wordmark needs seven glyphs; a hand-authored font is smaller than any new dependency and matches the mascot's pixel grid exactly.

**Full-block 5-row font** — the first pass shipped chunky letterforms that read as mushy at terminal scale; pairing ten glyph rows into half-block cells doubles the vertical resolution for crisp curves at the same five terminal rows.

## Consequences

Fresh installs greet with the whale-and-star hero under the oversized block-letter wordmark; the fox is one persistent `/welcome fox` away. Adding `welcome` to the builtin command names makes the `/w` prefix ambiguous with `/workflow` (prefix resolution rejects it; `/we` disambiguates). The welcome golden snapshot under `examples/tui` now records the two-step whale surface, the half-block wordmark, and the lineage line, and the static gates gain `verify-welcome-whale` beside `verify-welcome-fox`. The background-keyed cutout keeps the star's dark aura, which reads as a glow rim on dark themes and a faint dark halo on light ones — accepted at band scale. The command body and the art selection live in `welcome-mascot-command.ts` and `welcome-mascot-art.ts`; the app assembly carries only the registration call, one option/field pair, and the settle-time lookup, a residual +6 lines that raise the `app.ts` source-budget ceiling to 6191 — further thinning belongs to the C4 split track, not this feature. Welcome-surface test anchors standardize on the hero-unique `Oh My` text row (present in both brand modes); `█`-based negative anchors are unusable because the live region's input cursor is a full block.
...
- Renderer coverage pins whale and fox band geometry for all three widths, glyph hygiene, and width validation; hero coverage pins the half-block wordmark at 40+ detail columns, the one-line text fallback in the compact path, and the unchanged hero row budget; app coverage pins the whale default against the explicit fox option, and `/welcome` echo, rejection, and prefs persistence; runner coverage pins fail-loud `welcomeMascot` validation.
