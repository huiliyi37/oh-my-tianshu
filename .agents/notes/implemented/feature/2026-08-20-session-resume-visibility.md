...
- The crash-repair notice keys off the durable `turn/end { reason: { kind: 'interrupted' } }` marker: repair.ts is its only producer and the loop never emits it, so its presence in the log is the authoritative repair signal. No new session event, no persistence/loop write-path change.
...
- Pure projections: formatRestorablePickerList, corrupt-row formatting, wasCrashRepaired, interrupted transcript rows, orphan tool rows (restore-session / transcript / render specs).
- App-level: welcome list + digit routing + phase exit, banner/divider/crash notice on resume, --session attach (known/unknown id), picker summary rows, single-session tab, tips dynamics (app.spec).
- Backend: JSONL list keeps corrupt artifacts as version -1 with decoded ids; coordinator version error carries guidance (jsonl.spec / coordinator-contract).
