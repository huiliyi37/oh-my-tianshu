...
- **Tool-input rewrite.** A CC/Codex `updatedInput` is honored for loop-dispatched calls through the pre-commit phase ([the pre-tool-input-rewrite Agent Note](2026-06-30-pre-tool-input-rewrite.md)): history, the `tool/call` audit (with the original on a sidecar), presentation, and execution all carry the effective arguments; calls fired outside the loop phase keep the degraded warning.
