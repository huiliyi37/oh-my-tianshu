...
- `80 ≤ cols < 105`: 56 band, title and peer stay on the right of the fox
- `cols ≥ 105` and enough rows for `72×27` cells plus chrome: 72 band

The identity column starts with one theme-colored line of `Oh My Tianshu` beside the fox. There is no five-row block wordmark and no title stacked above the art. Peer copy stays `DeepSeek ◆ Tianshu Harness`.
...
Bought: the 56-band fox stays recognizable and keeps the title on the right; 72 is sharper at 105; attach no longer spends 3.24s on an opening.
...
- Formatter coverage pins half-block-only welcome rendering, rejected widths, and 56-beside-title at 80 / 105 wide / text fallback.
