...
## Implementation notes: best-of-N planning

PLAN may fan out to N parallel candidate planners over the same SPEC (`planCandidates`, 1..5, default 1 — the single-plan path is byte-identical to the original machine). When N > 1 a SELECT phase between PLAN and CRITIQUE runs an independent fresh-context judge that sees only SPEC.md plus the candidate texts — never any planner's reasoning — and scores them against a fixed rubric (correctness against the acceptance checks, feasibility, scope fit, risk), returning `{winner, rationale, mergeHints?}`. Selection is never done by a planner itself: self-evaluation bias is exactly what the fresh-context rule exists to remove. Candidates land as `PLAN-1.md … PLAN-N.md` (each bounded by `maxCandidateChars`), the winner becomes `PLAN.md`, and the judge's decision record is stored as `SELECTION.md` rather than appended to the plan, keeping PLAN.md the planner's own text. An out-of-range or non-integer winner fails the run loud at the boundary. The `select` phase event carries `{candidates, winner, rationale}` so selection is auditable from the log; the judge rides the `critique` effort entry unless a deployment maps `select` itself. CRITIQUE then runs on the selected PLAN.md unchanged, and a critique revision round re-fans-out N candidates plus a fresh selection.

## Implementation notes: background runs with in-conversation progress (2026-08-18)

The command handler originally ran the whole pipeline synchronously: the invoking surface blocked for minutes and phase work was invisible until the final result. The handler now runs a synchronous capability preflight (misconfiguration still answers at command time), starts the machine in the background, and acknowledges at once; every transcript line also lands as an appended plugin notice message (`form: 'notice'`, tail append — cache-safe, model-visible ⟺ logged), so progress, the final summary, and failures are visible in the conversation as they happen. Tests await the background run through the exported `pendingRuns` map.
