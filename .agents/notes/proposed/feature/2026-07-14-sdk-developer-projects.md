...
```sh
npm create @huiliyi37/sdk my-agent
cd my-agent
...
| `hmr` | default | `default` | Loads `@huiliyi37/cordis-plugin-hmr`; dev and start both enable it with the plugin defaults |
...
- `npm create @huiliyi37/sdk` collects project identity, provider, interface, features, an optional local plugin, package manager, and installation choice in the documented order, and cancellation leaves the target path absent
