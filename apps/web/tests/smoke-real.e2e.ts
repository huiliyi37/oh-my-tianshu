...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    ...
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /tianshu web: (http:\/\/[^\s]+)/.exec(out)
...
describe('dsh web keyless CLI smoke', () => {
  ...
  it('routes --dev runtime context and workspace instructions through the real CLI request', async () => {
    ...
    const child = spawn(
      process.execPath,
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0', '--dev'],
      {
        cwd: workspace,
        env: {
          ...process.env,
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key, W5)', () => {
  ...
  beforeAll(async () => {
    ...
    child = spawn(
      process.execPath,
      [
        '--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', String(port),
  ...
  it('2+3 empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () => document.title !== 'Tianshu Harness' && document.title.endsWith(' — Tianshu Harness'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Tianshu Harness`,
      durableTitle,
      { timeout: 15_000 },
    )
