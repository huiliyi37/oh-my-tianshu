/** Assembled keyless snapshot for the default `tianshu web` browser handoff. */
...
const openingMessage = 'tianshu web: opening the default browser; pass --no-open to disable'
...
if (process.env.DSH_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
  throw new Error('tianshu web browser-open snapshot requires built CLI and Web artifacts in lib mode')
}
...
describe.skipIf(!builtArtifactsExist)('tianshu web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    ...
    const readyUrl = /tianshu web: (http:\/\/[^\s]+)/u.exec(result.stdout)?.[1]
    const openLine = result.stdout.split('\n').find(line => line.startsWith('tianshu browser-open: '))
    const opening = result.stdout.includes(openingMessage)
    if (readyUrl === undefined || openLine === undefined || !opening) {
      throw new Error(`tianshu web browser-open evidence missing\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    const opened = JSON.parse(openLine.slice('tianshu browser-open: '.length)) as BrowserOpenRecord
  ...
  it('prints the launcher reason and manual URL after the Web app is ready', async () => {
    ...
    expect({
      diagnostic,
      exitCode: result.exitCode,
      opened: result.stdout.includes('tianshu browser-open: '),
  ...
  it('rejects a project browser command before starting the Web app', async () => {
    ...
    expect({
      ...
      opening: result.stdout.includes(openingMessage),
      opened: result.stdout.includes('tianshu browser-open: '),
      ready: result.stdout.includes('tianshu web: '),
    }).toMatchInlineSnapshot(`
      {
